<?php
// ==============================================================================
// SERIE A - KADRO VE TAKTİK MERKEZİ (BLUE & GREEN ITALIAN THEME)
// ==============================================================================
include '../db.php';
...
if (isset($_POST['taktik_kaydet'])) {
    ...
    $stmt = $pdo->prepare("UPDATE it_takimlar SET dizilis=?, oyun_tarzi=?, pres=?, tempo=? WHERE id=?");
    $stmt->execute([$dizilis, $oyun_tarzi, $pres, $tempo, $kullanici_takim_id]);
    
    $mesaj = "İtalyan taktikleri başarıyla güncellendi! Forza!";
    $mesaj_tipi = "success";
...
if (isset($_GET['islem']) && isset($_GET['oyuncu_id'])) {
    ...
    $benim_mi = $pdo->query("SELECT COUNT(*) FROM it_oyuncular WHERE id=$oyuncu_id AND takim_id=$kullanici_takim_id")->fetchColumn();
    
    if ($benim_mi) {
        if ($islem == 'ilk11_yap') {
            $ilk11_sayisi = $pdo->query("SELECT COUNT(*) FROM it_oyuncular WHERE takim_id=$kullanici_takim_id AND ilk_11=1")->fetchColumn();
            if ($ilk11_sayisi >= 11) {
                $mesaj = "Sahaya en fazla 11 oyuncu sürebilirsiniz! Önce birini yedeğe çekin.";
                $mesaj_tipi = "danger";
            } else {
                $pdo->exec("UPDATE it_oyuncular SET ilk_11=1, yedek=0 WHERE id=$oyuncu_id");
            }
        } elseif ($islem == 'yedek_yap') {
            $pdo->exec("UPDATE it_oyuncular SET ilk_11=0, yedek=1 WHERE id=$oyuncu_id");
        } elseif ($islem == 'kadro_disi_yap') {
            $pdo->exec("UPDATE it_oyuncular SET ilk_11=0, yedek=0 WHERE id=$oyuncu_id");
        }
        
        if(!$mesaj) {
            header("Location: sa_kadro.php"); 
            exit;
        }
    }
}
...
$tum_kadro = $pdo->query("SELECT * FROM it_oyuncular WHERE takim_id = $kullanici_takim_id ORDER BY CASE mevki WHEN 'K' THEN 1 WHEN 'D' THEN 2 WHEN 'OS' THEN 3 WHEN 'F' THEN 4 END, ovr DESC")->fetchAll(PDO::FETCH_ASSOC);
...
$takim_kalitesi = $pdo->query("SELECT AVG(ovr) FROM it_oyuncular WHERE takim_id = $kullanici_takim_id AND ilk_11 = 1")->fetchColumn();
...
<head>
    ...
    <title>Kadro & Taktik | Serie A</title>
    ...
    <style>
        ...
        .btn-taktik { background: var(--it-secondary); color: var(--it-dark); font-weight: 800; border: none; padding: 10px; width: 100%; border-radius: 6px; transition: 0.3s;}
        .btn-taktik:hover { background: #34d399; color: #000; box-shadow: 0 0 15px var(--it-secondary); transform: translateY(-2px);}
        ...
        .action-icon.up:hover { color: var(--it-secondary); transform: translateY(-2px); text-shadow: 0 0 5px var(--it-secondary);}
        ...
        .status-injured { background: rgba(16,185,129,0.2); color: var(--color-loss); border: 1px solid var(--color-loss);}
        .status-banned { background: rgba(5,150,105,0.2); color: var(--color-warning); border: 1px solid var(--color-warning);}
        
        .form-select { background-color: rgba(0,0,0,0.3); border: 1px solid var(--border-color); color: #fff; font-weight: 600;}
        .form-select:focus { border-color: var(--it-secondary); box-shadow: 0 0 0 0.25rem rgba(5,150,105,0.25); background-color: rgba(0,0,0,0.5); color: #fff;}
    </style>
</head>
<body>
    ...
    <nav class="pro-navbar">
        ...
        <div class="nav-menu d-none d-lg-flex gap-3">
            ...
            <a href="sa_kadro.php" class="nav-link-item text-white fw-bold" style="text-shadow: 0 0 10px var(--it-secondary);"><i class="fa-solid fa-users"></i> Kadro / Taktik</a>
        </div>

        <div class="d-flex gap-3">
            <a href="serie_a.php" class="nav-link-item text-danger border-danger" style="border: 1px solid; padding: 8px 20px; border-radius: 4px;">
    ...
    <div class="hero-banner">
        ...
        <h1 class="font-oswald m-0 text-white" style="font-size: 3rem; text-shadow: 0 0 20px rgba(5,150,105,0.3);"><?= htmlspecialchars($takim['takim_adi']) ?></h1>
        <p class="fw-bold mt-2" style="color: var(--it-secondary);">
        ...
        <div class="row g-4">
            <div class="col-xl-3 col-lg-4">
                <div class="panel-card p-4">
                    <h3 class="font-oswald mb-4 text-center border-bottom pb-3" style="border-color:var(--border-color); color:var(--it-primary);">
                        <i class="fa-solid fa-chess-board"></i> Taktik Odası
                    </h3>
                    
                    <form method="POST">
                        <div class="mb-4">
                            <label class="form-label fw-bold text-uppercase" style="font-size:0.8rem; color:var(--it-secondary);">Diziliş (Formasyon)</label>
                        ...
                        <div class="mb-4">
                            <label class="form-label fw-bold text-uppercase" style="font-size:0.8rem; color:var(--it-secondary);">Oyun Tarzı</label>
                        ...
                        <div class="mb-4">
                            <label class="form-label fw-bold text-uppercase" style="font-size:0.8rem; color:var(--it-secondary);">Takım Presi</label>
                            <select name="pres" class="form-select">
                                ...
                                <option value="Yüksek" <?= ($takim['pres'] ?? '')==='Yüksek'?'selected':'' ?>>Aggressivo (Şok Pres)</option>
                            </select>
                        </div>
                        <div class="mb-4">
                            <label class="form-label fw-bold text-uppercase" style="font-size:0.8rem; color:var(--it-secondary);">Oyun Temposu</label>
                            <select name="tempo" class="form-select">
                                ...
                                <option value="Çok Hızlı" <?= ($takim['tempo'] ?? '')==='Çok Hızlı'?'selected':'' ?>>İtalyan Temposu (Çok Hızlı)</option>
                            </select>
                        </div>
            ...
            <div class="col-xl-9 col-lg-8">
                ...
                <div class="panel-card mb-4" style="border-color: var(--it-secondary);">
                    <div class="panel-header text-white" style="background: rgba(5, 150, 105, 0.1); border-bottom-color: var(--it-secondary);">
                        <i class="fa-solid fa-shirt" style="color:var(--it-secondary);"></i> TITOLARI (İlk 11)
                    </div>
                ...
                <div class="panel-card mb-4" style="border-color: rgba(251, 191, 36, 0.4);">
                    <div class="panel-header text-warning" style="background: rgba(251, 191, 36, 0.1); border-bottom-color: rgba(251, 191, 36, 0.3);">
                        <i class="fa-solid fa-chair"></i> PANCHINA (Yedek Kulübesi)
                    </div>
                ...
                <div class="panel-card" style="border-color: rgba(16, 185, 129, 0.3);">
                    <div class="panel-header" style="background: rgba(16, 185, 129, 0.05); color: var(--it-primary); border-bottom-color: rgba(16, 185, 129, 0.2);">
                        <i class="fa-solid fa-ban"></i> FUORI ROSA (Kadro Dışı)
                    </div>
                    <div class="table-responsive" style="max-height: 350px; overflow-y:auto;">
                        <table class="data-table">
                            <thead>
    ...
    <div class="d-flex d-lg-none fixed-bottom p-2 justify-content-around align-items-center border-top" style="background: rgba(13,13,13,0.95); backdrop-filter: blur(10px); z-index:2000; padding-bottom: 15px !important; border-top-color: var(--it-secondary) !important;">
        ...
        <a href="sa_kadro.php" class="text-decoration-none text-center fw-bold" style="font-size: 0.8rem; width: 33%; color: var(--it-secondary);">
            <i class="fa-solid fa-users fs-5 mb-1 d-block"></i> Kadro
        </a>
        <a href="sa_transfer.php" class="text-secondary text-decoration-none text-center" style="font-size: 0.8rem; width: 33%;">
