<?php
// ==============================================================================
// SERIE A - MEDYA, PSİKOLOJİ VE BASIN ODASI (BLUE & GREEN ITALIAN THEME)
// ==============================================================================
include '../db.php';
...
sutunEkle($pdo, 'it_oyuncular', 'moral', 'INT DEFAULT 80');
sutunEkle($pdo, 'it_ayar', 'son_basin_haftasi', 'INT DEFAULT 0');
...
if(isset($_POST['moralleri_guncelle'])) {
    $pdo->exec("UPDATE it_oyuncular SET moral = LEAST(100, moral + 5) WHERE takim_id = $kullanici_takim_id AND ilk_11 = 1");
    $pdo->exec("UPDATE it_oyuncular SET moral = GREATEST(10, moral - 15) WHERE takim_id = $kullanici_takim_id AND yedek = 1 AND ovr >= 80");
    $pdo->exec("UPDATE it_oyuncular SET moral = GREATEST(10, moral - 5) WHERE takim_id = $kullanici_takim_id AND yedek = 1 AND ovr < 80");
    
    $mesaj = "İtalyan medyası yedek kalan yıldızları konuşuyor! Moraller güncellendi.";
    $mesaj_tipi = "warning";
}

// --- TAKIMA İTALYAN YEMEĞİ ---
if(isset($_POST['takim_yemegi'])) {
    $yemek_maliyeti = 800000; // 800K Euro
    if($takim['butce'] >= $yemek_maliyeti) {
        $pdo->exec("UPDATE it_takimlar SET butce = butce - $yemek_maliyeti WHERE id = $kullanici_takim_id");
        $pdo->exec("UPDATE it_oyuncular SET moral = LEAST(100, moral + 20) WHERE takim_id = $kullanici_takim_id");
        $mesaj = "Milano'nun en gözde restoranında Pasta ve Risotto şöleni! (-€800K) Takım morali yukarı!";
        $mesaj_tipi = "success";
...
if(isset($_POST['cevap_ver'])) {
    ...
    if(($ayar['son_basin_haftasi'] ?? 0) == $guncel_hafta) {
        $mesaj = "Bu hafta zaten Gazzetta'ya konuştunuz! Haftaya tekrar gelin."; $mesaj_tipi = "warning";
    } else {
        if($cevap_id == 1) {
            $pdo->exec("UPDATE it_oyuncular SET moral = GREATEST(10, moral - 10) WHERE takim_id = $kullanici_takim_id");
            $pdo->exec("UPDATE it_takimlar SET butce = butce + 1500000 WHERE id = $kullanici_takim_id");
            $mesaj = "Sert tutumunuz Serie A yönetiminden destek gördü (+€1.5M) ama oyuncular gerildi (-10 Moral).";
        } elseif($cevap_id == 2) {
            $pdo->exec("UPDATE it_oyuncular SET moral = LEAST(100, moral + 15) WHERE takim_id = $kullanici_takim_id");
            $mesaj = "Takımınızı Gazzetta'nın önünde savundunuz! Soyunma odasında harika bir atmosfer var (+15 Moral)!";
        } elseif($cevap_id == 3) {
...
$oyuncular = $pdo->query("SELECT * FROM it_oyuncular WHERE takim_id = $kullanici_takim_id ORDER BY moral ASC")->fetchAll(PDO::FETCH_ASSOC);
$ortalama_moral = $pdo->query("SELECT AVG(moral) FROM it_oyuncular WHERE takim_id = $kullanici_takim_id")->fetchColumn();
...
<head>
    ...
    <title>Presse & Psychologie | Serie A</title>
    ...
    <style>
        ...
        .nav-link-item:hover { color: var(--it-secondary); }
        ...
        .btn-action-primary { background: var(--it-secondary); color: var(--it-dark); font-weight: 900; padding: 10px 20px; border-radius: 4px; text-decoration: none; border: none; transition: 0.3s;}
        .btn-action-primary:hover { background: #34d399; color: #000; transform: translateY(-2px); box-shadow: 0 5px 15px rgba(5,150,105,0.4);}
        ...
        .press-option:hover { background: rgba(5,150,105,0.1); border-color: var(--it-secondary); padding-left: 25px;}
        ...
        .crisis-alert { animation: pulse 2s infinite; border-left: 4px solid var(--it-primary); background: rgba(16,185,129,0.1);}
...
<body>
    ...
    <nav class="pro-navbar">
        ...
        <div class="nav-menu d-none d-lg-flex gap-3">
            ...
            <a href="sa_basin.php" class="nav-link-item text-white fw-bold" style="text-shadow: 0 0 10px var(--it-secondary);"><i class="fa-solid fa-microphone"></i> Medya & Psikoloji</a>
        </div>
    </nav>
    ...
    <div class="hero-banner">
        <div class="hero-content container" style="max-width: 1400px;">
            <div class="row align-items-center">
                <div class="col-md-8">
                    <h1 class="font-oswald m-0 text-white" style="font-size: 3.5rem;">CONFERENZA STAMPA (BASIN ODASI)</h1>
                    <p class="text-white fs-5 mt-2 fw-bold opacity-75">Gazzetta, Corriere dello Sport, Tuttosport... İtalyan basını her şeyi konuşuyor. Sözlerini dikkatli seç.</p>
                </div>
                <div class="col-md-4 text-end">
                    <div class="badge bg-dark border p-3 text-start d-inline-block" style="border-color: var(--it-secondary) !important;">
                        ...
                        <div class="d-flex align-items-center gap-3">
                            ...
                            <?php $bg_color = $ortalama_moral >= 70 ? '#22c55e' : ($ortalama_moral >= 40 ? '#059669' : '#10b981'); ?>
    ...
    <div class="container py-5" style="max-width: 1400px;">
        ...
        <?php if($mesaj): ?>
            <div class="alert fw-bold text-center border-0 shadow-lg mb-4" style="background: <?= $mesaj_tipi == 'success' ? '#22c55e' : ($mesaj_tipi == 'warning' ? '#10b981' : 'var(--it-primary)') ?>; color: #000;">
                <?= $mesaj ?>
            </div>
        <?php endif; ?>
            ...
            <div class="col-xl-6">
                <div class="panel-card h-100">
                    <div class="panel-header" style="color: var(--it-secondary);">
                        <i class="fa-solid fa-microphone-lines me-2"></i> GAZZETTA CONFERENZA STAMPA (GIORNATA <?= $guncel_hafta ?>)
                    </div>
                    <div class="p-4">
                        <?php if(($ayar['son_basin_haftasi'] ?? 0) == $guncel_hafta): ?>
                            <div class="text-center py-5">
                                ...
                                <h4 class="font-oswald">CONFERENZA STAMPA TAMAMLANDI</h4>
                                <p class="text-muted">İtalyan basınına gerekli açıklamaları yaptınız.</p>
                            </div>
                        <?php else: ?>
                            <div class="mb-4">
                                <div class="d-flex gap-3 mb-3">
                                    <div style="width: 40px; height: 40px; border-radius: 50%; background: var(--it-primary); display:flex; align-items:center; justify-content:center;">
                                        <i class="fa-solid fa-camera text-white"></i>
                                    </div>
                                    <div style="background: rgba(255,255,255,0.1); padding: 15px; border-radius: 0 15px 15px 15px;">
                                        <p class="m-0 text-white fw-bold">"Allenatore, i tifosi chiedono risultati migliori e ci sono voci di tensioni nello spogliatoio. Come gestisce questa pressione?" <br><span class="text-muted small">(Teknik direktör, taraftarlar daha iyi sonuçlar bekliyor ve soyunma odasında gerilim var. Bu baskıyı nasıl yönetiyorsunuz?)</span></p>
                                    </div>
                                </div>
                            </div>
            ...
            <div class="col-xl-6">
                <div class="panel-card h-100">
                    <div class="panel-header d-flex justify-content-between align-items-center">
                        <span style="color: #fff;"><i class="fa-solid fa-face-angry text-danger me-2"></i> SPOGLIATOIO (Soyunma Odası)</span>
