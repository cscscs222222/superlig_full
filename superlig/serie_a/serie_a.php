<?php
// ==============================================================================
// SERIE A - ANA MERKEZ VE FİKSTÜR (BLUE & GREEN ITALIAN THEME)
// ==============================================================================
include '../db.php';

// Merkez Maç Motorunu Bağla (ÖN EK OLARAK 'it_' VERİYORUZ!)
if(file_exists('../MatchEngine.php')) {
    include '../MatchEngine.php';
    $engine = new MatchEngine($pdo, 'it_');
} else {
...
try {
    $pdo->exec("CREATE TABLE IF NOT EXISTS it_ayar ( id INT AUTO_INCREMENT PRIMARY KEY, hafta INT DEFAULT 1, sezon_yil INT DEFAULT 2025, kullanici_takim_id INT DEFAULT NULL )");
...
sutunEkle($pdo, 'it_takimlar', 'lig', "VARCHAR(50) DEFAULT 'Serie A'");
...
// --- SERIE A TAKIMLARINI OLUŞTUR (20 TAKIM) ---
$it_takim_sayisi = $pdo->query("SELECT COUNT(*) FROM it_takimlar")->fetchColumn();
if ($it_takim_sayisi < 20) {
    $italya_devleri = [
    ['Juventus',     'https://tmssl.akamaized.net/images/wappen/head/506.png',   88, 89],
    ['Inter Milan',  'https://tmssl.akamaized.net/images/wappen/head/46.png',    90, 90],
    ['AC Milan',     'https://tmssl.akamaized.net/images/wappen/head/5.png',     87, 88],
    ['Napoli',       'https://tmssl.akamaized.net/images/wappen/head/6195.png',  89, 86],
    ['Roma',         'https://tmssl.akamaized.net/images/wappen/head/12.png',    85, 84],
    ['Lazio',        'https://tmssl.akamaized.net/images/wappen/head/398.png',   85, 83],
    ['Fiorentina',   'https://tmssl.akamaized.net/images/wappen/head/430.png',   82, 81],
    ['Atalanta',     'https://tmssl.akamaized.net/images/wappen/head/800.png',   88, 83],
    ['Torino',       'https://tmssl.akamaized.net/images/wappen/head/416.png',   79, 80],
    ['Bologna',      'https://tmssl.akamaized.net/images/wappen/head/1025.png',  81, 82],
    ['Udinese',      'https://tmssl.akamaized.net/images/wappen/head/410.png',   74, 76],
    ['Sassuolo',     'https://tmssl.akamaized.net/images/wappen/head/6574.png',  76, 74],
    ['Empoli',       'https://tmssl.akamaized.net/images/wappen/head/749.png',   73, 74],
    ['Verona',       'https://tmssl.akamaized.net/images/wappen/head/276.png',   74, 75],
    ['Monza',        'https://tmssl.akamaized.net/images/wappen/head/3524.png',  76, 77],
    ['Lecce',        'https://tmssl.akamaized.net/images/wappen/head/1238.png',  72, 73],
    ['Genoa',        'https://tmssl.akamaized.net/images/wappen/head/252.png',   75, 76],
    ['Cagliari',     'https://tmssl.akamaized.net/images/wappen/head/1717.png',  73, 74],
    ['Frosinone',    'https://tmssl.akamaized.net/images/wappen/head/3629.png',  71, 72],
    ['Salernitana',  'https://tmssl.akamaized.net/images/wappen/head/2697.png',  69, 70],
];
    
    foreach($italya_devleri as $d) {
        $ad = $d[0]; $logo = $d[1]; $huc = $d[2]; $sav = $d[3];
        $var_mi = $pdo->query("SELECT COUNT(*) FROM it_takimlar WHERE takim_adi = " . $pdo->quote($ad))->fetchColumn();
        if($var_mi == 0) {
            $butce = rand(20000000, 200000000);
            $stmt = $pdo->prepare("INSERT INTO it_takimlar (takim_adi, logo, hucum, savunma, butce, lig) VALUES (?, ?, ?, ?, ?, 'Serie A')");
            ...
            for($i=0; $i<22; $i++) {
                ...
                $yedek = ($i >= 11 && $i < 18) ? 1 : 0;
                $fiyat = ($ovr * $ovr) * 4000;
                $stmt2 = $pdo->prepare("INSERT INTO it_oyuncular (takim_id, isim, mevki, ovr, yas, fiyat, lig, ilk_11, yedek) VALUES (?, ?, ?, ?, ?, ?, 'Serie A', ?, ?)");
...
$takimlar_listesi = $pdo->query("SELECT id, takim_adi, hucum, savunma FROM it_takimlar")->fetchAll(PDO::FETCH_ASSOC);
foreach($takimlar_listesi as $t) {
    $oyuncu_sayisi = $pdo->query("SELECT COUNT(*) FROM it_oyuncular WHERE takim_id = {$t['id']}")->fetchColumn();
    if($oyuncu_sayisi == 0) {
        for($i=0; $i<22; $i++) {
            ...
            $yedek = ($i >= 11 && $i < 18) ? 1 : 0;
            $fiyat = ($ovr * $ovr) * 4000;
            $stmt = $pdo->prepare("INSERT INTO it_oyuncular (takim_id, isim, mevki, ovr, yas, fiyat, lig, ilk_11, yedek) VALUES (?, ?, ?, ?, ?, ?, 'Serie A', ?, ?)");
...
function garanti_olay_uret($pdo, $takim_id, $skor) {
    $oyuncular = $pdo->query("SELECT isim FROM it_oyuncular WHERE takim_id = $takim_id AND ilk_11 = 1")->fetchAll(PDO::FETCH_COLUMN);
    if(empty($oyuncular)) $oyuncular = $pdo->query("SELECT isim FROM it_oyuncular WHERE takim_id = $takim_id")->fetchAll(PDO::FETCH_COLUMN);
...
$max_hafta = 38; // Serie A 20 takım, 38 haftadır.

// --- FİKSTÜR OLUŞTURMA ---
$mac_sayisi = 0;
try { $mac_sayisi = $pdo->query("SELECT COUNT(*) FROM it_maclar WHERE sezon_yil = $sezon_yili")->fetchColumn(); } catch(Throwable $e){}

if($mac_sayisi == 0) {
    $takimlar = $pdo->query("SELECT id FROM it_takimlar ORDER BY RAND()")->fetchAll(PDO::FETCH_COLUMN);
    if(count($takimlar) > 1) {
        ...
        for ($h = 1; $h <= $yari; $h++) {
            for ($i = 0; $i < $m_sayisi; $i++) {
                ...
                if ($ev != 0 && $dep != 0) {
                    if ($i % 2 == 0) { 
                        $pdo->exec("INSERT INTO it_maclar (ev, dep, hafta, sezon_yil) VALUES ($ev, $dep, $h, $sezon_yili)"); 
                        $pdo->exec("INSERT INTO it_maclar (ev, dep, hafta, sezon_yil) VALUES ($dep, $ev, ".($h+$yari).", $sezon_yili)"); 
                    } else { 
                        $pdo->exec("INSERT INTO it_maclar (ev, dep, hafta, sezon_yil) VALUES ($dep, $ev, $h, $sezon_yili)"); 
                        $pdo->exec("INSERT INTO it_maclar (ev, dep, hafta, sezon_yil) VALUES ($ev, $dep, ".($h+$yari).", $sezon_yili)"); 
                    }
                }
            }
            $son = array_pop($takimlar); array_splice($takimlar, 1, 0, [$son]);
        }
        $pdo->exec("INSERT INTO it_haberler (hafta, metin, tip) VALUES (1, 'Serie A Başlıyor! İtalya''da nefesler tutuldu.', 'sistem')");
        header("Location: serie_a.php"); exit;
    }
}
...
if(isset($_GET['action'])) {
    ...
    if($action == 'takim_sec' && isset($_GET['tid'])) {
        $tid = (int)$_GET['tid'];
        $pdo->exec("UPDATE it_ayar SET kullanici_takim_id = $tid WHERE id=1");
        header("Location: serie_a.php"); exit;
    }
    ...
    if($action == 'tek_mac_simule' && isset($_GET['mac_id'])) {
        ...
        $m = $pdo->query("SELECT m.id, m.ev, m.dep, t1.takim_adi as ev_ad, t2.takim_adi as dep_ad, t1.hucum as ev_hucum, t1.savunma as ev_savunma, t2.hucum as dep_hucum, t2.savunma as dep_savunma 
                           FROM it_maclar m JOIN it_takimlar t1 ON m.ev=t1.id JOIN it_takimlar t2 ON m.dep=t2.id 
        ...
        if($m) {
            ...
            $stmt = $pdo->prepare("UPDATE it_maclar SET ev_skor=?, dep_skor=?, ev_olaylar=?, dep_olaylar=?, ev_kartlar=?, dep_kartlar=? WHERE id=?");
            $stmt->execute([$ev_skor, $dep_skor, $ev_detay['olaylar'], $dep_detay['olaylar'], $ev_detay['kartlar'], $dep_detay['kartlar'], $m['id']]);
            
            $pdo->exec("UPDATE it_takimlar SET atilan_gol = atilan_gol + $ev_skor, yenilen_gol = yenilen_gol + $dep_skor WHERE id = {$m['ev']}");
            $pdo->exec("UPDATE it_takimlar SET atilan_gol = atilan_gol + $dep_skor, yenilen_gol = yenilen_gol + $ev_skor WHERE id = {$m['dep']}");
            
            if($ev_skor > $dep_skor) { $pdo->exec("UPDATE it_takimlar SET puan=puan+3, galibiyet=galibiyet+1 WHERE id={$m['ev']}"); $pdo->exec("UPDATE it_takimlar SET malubiyet=malubiyet+1 WHERE id={$m['dep']}"); }
            elseif($ev_skor == $dep_skor) { $pdo->exec("UPDATE it_takimlar SET puan=puan+1, beraberlik=beraberlik+1 WHERE id IN ({$m['ev']}, {$m['dep']})"); }
            else { $pdo->exec("UPDATE it_takimlar SET puan=puan+3, galibiyet=galibiyet+1 WHERE id={$m['dep']}"); $pdo->exec("UPDATE it_takimlar SET malubiyet=malubiyet+1 WHERE id={$m['ev']}"); }
        }
        
        $kalan_mac = $pdo->query("SELECT COUNT(*) FROM it_maclar WHERE hafta = $hafta AND ev_skor IS NULL")->fetchColumn();
        if($kalan_mac == 0) { 
            $pdo->exec("UPDATE it_ayar SET hafta = LEAST($max_hafta, hafta + 1)"); 
        }
        header("Location: serie_a.php?hafta=$hedef_hafta"); exit;
    }
    ...
    if($action == 'sezonu_simule') {
        for($h = $hafta; $h <= $max_hafta; $h++) {
            $maclar_h = $pdo->query("SELECT m.id, m.ev, m.dep, t1.hucum as ev_hucum, t1.savunma as ev_savunma, t2.hucum as dep_hucum, t2.savunma as dep_savunma FROM it_maclar m JOIN it_takimlar t1 ON m.ev=t1.id JOIN it_takimlar t2 ON m.dep=t2.id WHERE m.hafta=$h AND m.ev_skor IS NULL")->fetchAll(PDO::FETCH_ASSOC);
            foreach($maclar_h as $m) {
                $skorlar = $engine->gercekci_skor_hesapla($m['ev'], $m['dep'], $m);
                $ev_skor = $skorlar['ev']; $dep_skor = $skorlar['dep'];
                $ev_detay = $engine->mac_olay_uret($m['ev'], $ev_skor);
                $dep_detay = $engine->mac_olay_uret($m['dep'], $dep_skor);
                $pdo->prepare("UPDATE it_maclar SET ev_skor=?,dep_skor=?,ev_olaylar=?,dep_olaylar=?,ev_kartlar=?,dep_kartlar=? WHERE id=?")->execute([$ev_skor,$dep_skor,$ev_detay['olaylar'],$dep_detay['olaylar'],$ev_detay['kartlar'],$dep_detay['kartlar'],$m['id']]);
                $pdo->exec("UPDATE it_takimlar SET atilan_gol=atilan_gol+$ev_skor,yenilen_gol=yenilen_gol+$dep_skor WHERE id={$m['ev']}");
                $pdo->exec("UPDATE it_takimlar SET atilan_gol=atilan_gol+$dep_skor,yenilen_gol=yenilen_gol+$ev_skor WHERE id={$m['dep']}");
                if($ev_skor>$dep_skor){$pdo->exec("UPDATE it_takimlar SET puan=puan+3,galibiyet=galibiyet+1 WHERE id={$m['ev']}");$pdo->exec("UPDATE it_takimlar SET malubiyet=malubiyet+1 WHERE id={$m['dep']}");}
                elseif($ev_skor==$dep_skor){$pdo->exec("UPDATE it_takimlar SET puan=puan+1,beraberlik=beraberlik+1 WHERE id IN ({$m['ev']},{$m['dep']})");}
        ...
        $pdo->exec("UPDATE it_ayar SET hafta=$max_hafta");
        header("Location: serie_a.php?sezon_bitti=1"); exit;
    }
}

// --- VERİ ÇEKİMİ ---
$puan_durumu = $pdo->query("SELECT * FROM it_takimlar ORDER BY puan DESC, (atilan_gol - yenilen_gol) DESC, atilan_gol DESC")->fetchAll(PDO::FETCH_ASSOC);
...
$haftanin_fiksturu = $pdo->query("SELECT m.*, t1.takim_adi as ev_ad, t1.logo as ev_logo, t2.takim_adi as dep_ad, t2.logo as dep_logo FROM it_maclar m JOIN it_takimlar t1 ON m.ev = t1.id JOIN it_takimlar t2 ON m.dep = t2.id WHERE m.hafta = $goster_hafta")->fetchAll(PDO::FETCH_ASSOC);
$yayinlanacak_maclar = array_filter($haftanin_fiksturu, function($m) { return $m['ev_skor'] === NULL; });

$benim_macim_id = null;
if($kullanici_takim) {
    $benim_macim_id = $pdo->query("SELECT id FROM it_maclar WHERE hafta=$goster_hafta AND ev_skor IS NULL AND (ev=$kullanici_takim OR dep=$kullanici_takim)")->fetchColumn();
}

$sezon_tamam = false;
$sampiyon_takim = null;
try {
    $toplam_mac_it = $pdo->query("SELECT COUNT(*) FROM it_maclar WHERE sezon_yil=$sezon_yili")->fetchColumn();
    $kalan_it = $pdo->query("SELECT COUNT(*) FROM it_maclar WHERE sezon_yil=$sezon_yili AND ev_skor IS NULL")->fetchColumn();
    if($toplam_mac_it > 0 && $kalan_it == 0 && $hafta >= $max_hafta) {
        $sezon_tamam = true;
...
<head>
    ...
    <style>
        ...
        :root {
            ...
            --it-accent: #34d399;    /* Açık Yeşil Vurgu */
            --it-dark: #0d0d0d;
        ...
        .btn-action-primary { background: var(--it-secondary); color: var(--it-dark); font-weight: 800; padding: 8px 20px; border-radius: 4px; text-decoration: none; border: none; transition: 0.3s;}
        .btn-action-primary:hover { background: #34d399; color: #000; box-shadow: 0 0 15px var(--it-secondary); }
        ...
        .data-table th { padding: 1rem; color: var(--it-secondary); font-weight: 700; text-transform: uppercase; font-size: 0.75rem; border-bottom: 1px solid var(--border-color); text-align: center; }
        ...
        /* Serie A Bölgeleme Kuralları */
        .data-table tbody tr td:first-child { border-left: 4px solid transparent; }
        .zone-cl td:first-child { border-left-color: var(--it-secondary) !important; background: rgba(5,150,105,0.05); }
        .zone-el td:first-child { border-left-color: #fb923c !important; }
        .zone-rel td:first-child { border-left-color: var(--it-primary) !important; opacity: 0.8;}

        /* FİKSTÜR */
        .fixture-wrapper { display: flex; flex-direction: column; gap: 15px; overflow-y: auto; padding: 1rem; flex: 1; }
        
        .scorebug-container { background: rgba(0,0,0,0.5); border: 1px solid rgba(16,185,129,0.2); border-radius: 10px; overflow: hidden; transition: 0.3s; width: 100%; flex-shrink: 0; }
        .scorebug-container:hover { border-color: var(--it-secondary); box-shadow: 0 5px 15px rgba(5,150,105,0.15); transform: translateY(-2px);}
        ...
        .match-score { font-family: 'Oswald', sans-serif; font-size: 1.8rem; font-weight: 900; color: var(--it-secondary); line-height: 1; letter-spacing: 1px; text-shadow: 0 2px 4px rgba(0,0,0,0.8); }
        ...
        .action-btn { flex: 1; padding: 10px; text-align: center; text-decoration: none; color: var(--it-secondary); font-size: 0.85rem; font-weight: 700; text-transform: uppercase; transition: 0.2s; display: flex; justify-content: center; align-items: center; gap: 8px;}
        .action-btn:hover { background: var(--it-secondary); color: var(--it-dark) !important; }
        
        /* GOL DETAYLARI */
        .events-grid { display: flex; width: 100%; background: rgba(0,0,0,0.8); border-top: 1px solid rgba(16,185,129,0.2); padding: 8px 0; font-size: 0.8rem; }
        ...
        .event-time { font-family: 'Oswald'; font-weight: 700; color: var(--it-primary); flex-shrink: 0; min-width: 25px;}
        ...
        .ref-card.yellow { background-color: #f87171; }
        ...
        @keyframes pulse_glow {
            0% { box-shadow: 0 0 10px rgba(5,150,105,0.5); }
            50% { box-shadow: 0 0 30px rgba(5,150,105,0.8), 0 0 50px rgba(16,185,129,0.3); }
            100% { box-shadow: 0 0 10px rgba(5,150,105,0.5); }
        }
    </style>
</head>
<body>
    ...
    <nav class="pro-navbar">
        <a href="serie_a.php" class="nav-brand">
            <img src="https://upload.wikimedia.org/wikipedia/en/thumb/d/df/Serie A_logo_%282017%29.svg/200px-Serie A_logo_%282017%29.svg.png"
                 alt="Serie A" class="league-logo"
                 onerror="this.style.display='none'">
            <span class="font-oswald">SERIE A</span>
        </a>
        
        <div class="nav-menu d-none d-lg-flex gap-3">
            <a href="../index.php" class="nav-link-item"><i class="fa-solid fa-house"></i> Merkez Hub</a>
            <a href="serie_a.php" class="nav-link-item text-white fw-bold" style="text-shadow: 0 0 10px var(--it-secondary);"><i class="fa-solid fa-tv"></i> Maç Merkezi</a>
            ...
            <a href="sa_basin.php" class="nav-link-item"><i class="fa-solid fa-microphone"></i> Medya</a>
            <a href="sa_tesisler.php" class="nav-link-item"><i class="fa-solid fa-building"></i> Tesisler</a>
        </div>
        ...
        <div class="d-flex gap-3">
            <?php if($kullanici_takim): ?>
                ...
                <?php if(!$sezon_tamam): ?>
                <a href="?action=sezonu_simule" class="btn-action-outline" style="background:#7f1d1d;border-color:#dc2626;color:#fca5a5;" onclick="return confirm('Serie A sezonu simüle edilecek. Devam?')">
    ...
    <div class="container-fluid py-3 px-4">
        <div style="background:linear-gradient(135deg,#0d0d0d,#064e3b);border:2px solid #10b981;border-radius:12px;padding:20px 24px;margin-bottom:16px;text-align:center;">
            <div style="font-family:monospace;color:#10b981;font-size:0.82rem;">=============================================</div>
            <div style="font-family:'Oswald',sans-serif;font-size:1.3rem;font-weight:900;color:#fff;margin:8px 0;">🏆 SERİE A <?=$sezon_yili?>/<?=$sezon_yili+1?> SEZONU ŞAMPİYONU 🏆</div>
            <div style="font-family:'Oswald',sans-serif;font-size:1.6rem;font-weight:900;color:#10b981;margin:6px 0;">★ <?=htmlspecialchars($sampiyon_takim['takim_adi'])?> ★</div>
            <div style="font-family:monospace;color:#10b981;font-size:0.82rem;">=============================================</div>
            ...
            <div style="margin-top:12px;font-size:0.8rem;max-width:400px;margin-left:auto;margin-right:auto;text-align:left;">
                <div style="color:#10b981;font-weight:700;margin-bottom:6px;text-transform:uppercase;font-size:0.72rem;">Final Tablosu — İlk 5</div>
                ...
                <div style="display:flex;gap:8px;padding:3px 0;color:#fff;border-bottom:1px solid rgba(255,255,255,0.1);">
                    <span style="width:20px;color:#10b981;font-weight:700;"><?=$idx+1?></span>
                    ...
                    <span style="color:#10b981;font-weight:900;"><?=$st['puan']?>P</span>
                </div>
                <?php endforeach; ?>
            </div>
            <div style="background:rgba(0,0,0,0.4);border:1px solid rgba(16,185,129,0.3);border-radius:6px;padding:10px 14px;margin-top:12px;font-family:monospace;font-size:0.78rem;color:#a3e635;text-align:left;max-width:400px;margin-left:auto;margin-right:auto;">
                <div style="color:#94a3b8;margin-bottom:4px;">// index.php için son şampiyon güncellemesi</div>
                $son_sampiyon['serie_a'] = "<?=htmlspecialchars($sampiyon_takim['takim_adi'])?>";
            </div>
        </div>
    </div>
    <?php endif; ?>
    ...
    <?php if(!$kullanici_takim): ?>
        <div class="container py-5 text-center" style="max-width: 1200px;">
            ...
            <h1 class="font-oswald mb-4" style="font-size: 4rem; color: var(--it-secondary); text-shadow: 0 0 20px rgba(5,150,105,0.3);">SERIE A'YA HOŞ GELDİN</h1>
            <p class="text-white mb-5 fs-5">İtalya'nın en prestijli liginde yönetmek istediğin kulübü seç.</p>
            <div class="row g-4 justify-content-center">
                <?php 
                $secilebilir = $pdo->query("SELECT * FROM it_takimlar ORDER BY takim_adi ASC")->fetchAll(PDO::FETCH_ASSOC);
                ...
                <div class="col-md-3 col-sm-4 col-6">
                    <div class="panel-card p-4 text-center hover-lift" onclick="window.location='?action=takim_sec&tid=<?= $t['id'] ?>';">
                        ...
                        <small style="color: var(--it-secondary);">Hücum: <?= $t['hucum'] ?> | Savunma: <?= $t['savunma'] ?></small>
                    </div>
                </div>
                <?php endforeach; ?>
            </div>
                ...
                <div class="col-xl-7">
                    <div class="panel-card" style="height: 100%;">
                        <div class="panel-header">
                            <span class="font-oswald text-white fs-5"><i class="fa-solid fa-list-ol me-2" style="color:var(--it-secondary);"></i> CLASSIFICA</span>
                        </div>
                                    ...
                                    <tr class="<?= $row_class ?>">
                                        ...
                                        <td>
                                            <div class="cell-club">
                                                ...
                                                <span style="<?= $t['id'] == $kullanici_takim ? 'color: var(--it-secondary);' : '' ?>"><?= htmlspecialchars($t['takim_adi']) ?></span>
                                            </div>
                                        </td>
                                        ...
                                        <td class="font-oswald fs-5" style="color: <?= $t['id'] == $kullanici_takim ? 'var(--it-secondary)' : '#fff' ?>;"><?= $t['puan'] ?></td>
                                    </tr>
                                    <?php endforeach; ?>
                                </tbody>
                            </table>
                        ...
                        <div class="p-3 border-top" style="border-color:var(--border-color); font-size:0.8rem; background:rgba(0,0,0,0.5);">
                            <span style="color:var(--it-secondary);"><i class="fa-solid fa-square"></i> 1-4: Şampiyonlar Ligi</span>
                            <span class="ms-3 text-warning"><i class="fa-solid fa-square"></i> 5-6: Avrupa Kupası</span>
                            <span class="ms-3" style="color:var(--it-primary);"><i class="fa-solid fa-square"></i> 18-20: Küme Düşme</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-xl-5">
                    <div class="panel-card" style="height: 100%; max-height:850px;">
                        <div class="panel-header">
                            ...
                            <span class="font-oswald text-white fs-5">GIORNATA <?= $goster_hafta ?></span>
                        ...
                        <div class="fixture-wrapper">
                            <?php foreach($yayinlanacak_maclar as $mac): ?>
                                <div class="scorebug-container">
                                    ...
                                    <div class="match-actions">
                                        <a href="../canli_mac.php?id=<?= $mac['id'] ?>&lig=it&hafta=<?= $goster_hafta ?>" class="action-btn text-success"><i class="fa-solid fa-satellite-dish"></i> CANLI İZLE</a>
                                    </div>
                                </div>
                            <?php endforeach; ?>
                            ...
                            ?>
                                <div class="scorebug-container" style="border-color: rgba(5,150,105,0.5); box-shadow: 0 0 10px rgba(5,150,105,0.1);">
                                    <div class="score-grid">
                                        <div class="team-block home">
                                            <span class="team-name"><?= htmlspecialchars($mac['ev_ad']) ?></span>
                                            <img src="<?= htmlspecialchars($mac['ev_logo']) ?>" class="team-logo">
                                    ...
                                    <div class="events-grid">
                                        <div class="event-col home">
                                            ...
                                            foreach($ev_olaylar as $o) {
                                                ...
                                                echo "<div class='event-item justify-content-end'>$asist <span class='event-player'>$oyuncu</span> <span class='event-time'>$dakika'</span> <i class='fa-solid fa-futbol' style='color:var(--it-secondary);'></i></div>"; 
                                                $yazilan_gol_ev++;
                                            }
                                        ...
                                        <div class="event-col away">
                                            ...
                                            foreach($dep_olaylar as $o) {
                                                ...
                                                $dakika = $o['dakika'] ?? rand(1,90);
                                                $asist = (isset($o['asist']) && $o['asist'] !== '-') ? "<span class='event-assist'>(A: " . htmlspecialchars($o['asist']) . ")</span>" : "";
                                                echo "<div class='event-item'><i class='fa-solid fa-futbol' style='color:var(--it-secondary);'></i> <span class='event-time'>$dakika'</span> <span class='event-player'>$oyuncu</span> $asist</div>"; 
                                                $yazilan_gol_dep++;
                                            }
    ...
    <?php if($kullanici_takim): ?>
    <div class="d-flex d-lg-none fixed-bottom p-2 justify-content-around align-items-center border-top" style="background: rgba(21,0,3,0.97); backdrop-filter: blur(10px); z-index:2000; padding-bottom: 15px !important; border-top-color: var(--it-secondary) !important;">
        <a href="serie_a.php" class="text-decoration-none text-center fw-bold" style="font-size: 0.8rem; width: 20%; color: var(--it-secondary);">
            <i class="fa-solid fa-tv fs-5 mb-1 d-block"></i> Fikstür
        </a>
        <a href="sa_kadro.php" class="text-secondary text-decoration-none text-center" style="font-size: 0.8rem; width: 20%;">
            <i class="fa-solid fa-users fs-5 mb-1 d-block text-white"></i> Kadro
        </a>
        <a href="sa_transfer.php" class="text-secondary text-decoration-none text-center" style="font-size: 0.8rem; width: 20%;">
            <i class="fa-solid fa-comments-dollar fs-5 mb-1 d-block text-white"></i> Transfer
        </a>
        <a href="sa_puan.php" class="text-secondary text-decoration-none text-center" style="font-size: 0.8rem; width: 20%;">
            <i class="fa-solid fa-chart-pie fs-5 mb-1 d-block text-white"></i> Veri
        </a>
        <a href="sa_basin.php" class="text-secondary text-decoration-none text-center" style="font-size: 0.8rem; width: 20%;">
    ...
    <div style="position:fixed; bottom: 80px; left:50%; transform:translateX(-50%); z-index: 3000;">
        <a href="sa_sezon_gecisi.php" class="btn fw-bold py-3 px-5" style="background:var(--it-secondary); color:var(--it-dark); border-radius:50px; box-shadow: 0 5px 20px rgba(5,150,105,0.5); animation: pulse_glow 2s infinite; font-size:1.1rem;">
