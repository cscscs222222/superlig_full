<?php
// ==============================================================================
// SERIE A - İSTATİSTİK VE VERİ MERKEZİ (BLUE & GREEN ITALIAN THEME)
// ==============================================================================
include '../db.php';
...
$takim_form = [];
$tum_oynanan_maclar_form = $pdo->query("SELECT ev, dep, ev_skor, dep_skor FROM it_maclar WHERE ev_skor IS NOT NULL ORDER BY hafta DESC")->fetchAll(PDO::FETCH_ASSOC);
...
$tum_maclar = $pdo->query("SELECT ev_olaylar, dep_olaylar, ev_kartlar, dep_kartlar FROM it_maclar WHERE ev_skor IS NOT NULL")->fetchAll(PDO::FETCH_ASSOC);
...
$oyuncu_kulupleri = [];
try {
    $oyuncular_db = $pdo->query("SELECT o.isim, t.logo, t.takim_adi FROM it_oyuncular o JOIN it_takimlar t ON o.takim_id = t.id")->fetchAll(PDO::FETCH_ASSOC);
...
<head>
    ...
    <title>Serie A Veri Merkezi | Manager</title>
    ...
    <style>
        ...
        .my-team-row td { background: rgba(5,150,105,0.1); font-weight: 800; color: var(--it-secondary) !important;}
        ...
        .leaderboard-item:hover { background: rgba(5,150,105,0.05); }
        .leaderboard-item:last-child { border-bottom: none; }
        
        .lb-rank { font-family: 'Oswald'; font-size: 1.2rem; font-weight: 700; width: 30px; color: var(--text-muted); text-align:center;}
        .lb-rank.first { color: var(--it-secondary); font-size: 1.5rem; text-shadow: 0 0 10px rgba(5,150,105,0.5);}
        ...
        .lb-stat { font-family: 'Oswald'; font-size: 1.3rem; font-weight: 900; color: var(--it-dark); background: var(--it-secondary); padding: 2px 12px; border-radius: 6px; box-shadow: 0 2px 5px rgba(0,0,0,0.5);}
        .lb-stat.assists { background: #f87171; }
        .lb-stat.cards { background: var(--it-primary); color: #fff;}
...
<body>
    ...
    <nav class="pro-navbar">
        ...
        <div class="nav-menu d-none d-lg-flex gap-3">
            ...
            <a href="sa_puan.php" class="nav-link-item text-white fw-bold" style="text-shadow: 0 0 10px var(--it-secondary);"><i class="fa-solid fa-chart-pie"></i> İstatistik</a>
        </div>
        
        <div class="d-flex gap-3">
            <a href="serie_a.php" class="btn-action-outline">
    ...
    <div class="hero-banner">
        <h1 class="font-oswald m-0 text-white" style="font-size: 3.5rem; text-shadow: 0 0 20px rgba(5,150,105,0.3);"><i class="fa-solid fa-chart-line" style="color:var(--it-secondary);"></i> SERIE A CENTRO DATI</h1>
        <p class="fs-5 mt-2 fw-bold" style="color: var(--text-muted);">Serie A tablosu, istatistik krallıkları ve kart bilgileri.</p>
    </div>
    ...
    <div class="container py-5" style="max-width: 1600px;">
        <div class="row g-4">
            ...
            <div class="col-xl-8 col-lg-7">
                <div class="panel-card" style="height: 100%;">
                    <div class="panel-header">
                        <span class="font-oswald text-white fs-5"><i class="fa-solid fa-list-ol me-2" style="color:var(--it-secondary);"></i> SERIE A TABELLE</span>
                        <span class="badge bg-dark border text-white" style="border-color: var(--it-primary) !important;">Giornata <?= $hafta > 38 ? 38 : $hafta ?></span>
                    </div>
                            ...
                            <tbody>
                                ...
                                <tr class="<?= $row_class ?>">
                                    <td class="font-oswald" style="font-size: 1.1rem; color: var(--text-muted);">
                                        <?= $sira == 1 ? '<i class="fa-solid fa-crown me-1" style="color:var(--it-secondary);"></i> 1' : $sira ?>
                                    </td>
                                    <td>
                                        <a href="sa_takim.php?id=<?= $t['id'] ?>" class="cell-club">
                    ...
                    <div class="px-4 py-3 border-top" style="border-color: var(--border-color); background: rgba(0,0,0,0.5); font-size:0.8rem; font-weight:600; color:var(--text-muted);">
                        <div class="d-flex flex-wrap gap-4 justify-content-center">
                            <div class="d-flex align-items-center gap-2"><div style="width:12px;height:12px;background:var(--it-secondary);border-radius:2px; box-shadow: 0 0 5px var(--it-secondary);"></div> 1-4: Şampiyonlar Ligi</div>
                            ...
                            <div class="d-flex align-items-center gap-2"><div style="width:12px;height:12px;background:var(--it-primary);border-radius:2px;"></div> 18-20: Küme Düşme</div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="col-xl-4 col-lg-5 d-flex flex-column gap-4">
                ...
                <div class="panel-card" style="border-color: rgba(5,150,105,0.3);">
                    <div class="panel-header" style="background: rgba(5,150,105,0.05); color: var(--it-secondary);">
                        <span class="font-oswald"><i class="fa-solid fa-futbol me-2"></i> CLASSIFICA MARCATORI (Gol Krallığı)</span>
                    </div>
                    <ul class="leaderboard-list">
                        ...
                        <?php $sira++; endforeach; ?>
                        <?php if(empty($top_gol)): ?><li class="p-4 text-center text-muted">Serie A'da henüz gol atılmadı.</li><?php endif; ?>
                    </ul>
                </div>
                ...
                <div class="panel-card" style="border-color: rgba(251,191,36,0.3);">
                    <div class="panel-header" style="background: rgba(251,191,36,0.05); color: #f87171;">
                        <span class="font-oswald"><i class="fa-solid fa-shoe-prints me-2"></i> CLASSIFICA ASSIST (Asist Krallığı)</span>
                    </div>
                    <ul class="leaderboard-list">
                        <?php 
                        $sira = 1;
                ...
                <div class="panel-card" style="border-color: rgba(16,185,129,0.3);">
                    <div class="panel-header" style="color: var(--it-primary); background: rgba(16,185,129,0.05);">
                        <span class="font-oswald"><i class="fa-solid fa-square"></i> CARTELLINI ROSSI (Kırmızı Kartlar)</span>
                    </div>
                    <ul class="leaderboard-list">
                        <?php 
                        $sira = 1;
    ...
    <div class="d-flex d-lg-none fixed-bottom p-2 justify-content-around align-items-center border-top" style="background: rgba(13,13,13,0.95); backdrop-filter: blur(10px); z-index:2000; padding-bottom: 15px !important; border-top-color: var(--it-secondary) !important;">
        ...
        <a href="sa_puan.php" class="text-decoration-none text-center fw-bold" style="font-size: 0.8rem; width: 25%; color: var(--it-secondary);">
