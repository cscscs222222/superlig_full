<?php
// ==============================================================================
// COUPE DE FRANCE - FRANSIZ KUPA TURNUVASI
// Türkiye Kupası / FA Cup ile aynı mantık, Ligue 1 takımları ile
// ==============================================================================
include '../db.php';
...
// TABLOLARI HAZIRLA
try {
    $pdo->exec("CREATE TABLE IF NOT EXISTS cdf_maclar (
        id INT AUTO_INCREMENT PRIMARY KEY,
        ev INT, dep INT,
        ev_skor INT DEFAULT NULL, dep_skor INT DEFAULT NULL,
        tur VARCHAR(30) DEFAULT 'Ön Eleme',
    ...
    $pdo->exec("CREATE TABLE IF NOT EXISTS cdf_ayar (
        id INT AUTO_INCREMENT PRIMARY KEY,
        mevcut_tur VARCHAR(30) DEFAULT 'Tur 1',
        sezon_yil INT DEFAULT 2025,
        sampiyon VARCHAR(255) DEFAULT NULL
    )");
    $ayar_sayisi = $pdo->query("SELECT COUNT(*) FROM cdf_ayar")->fetchColumn();
    if($ayar_sayisi == 0) $pdo->exec("INSERT INTO cdf_ayar (mevcut_tur, sezon_yil) VALUES ('Tur 1', 2025)");
} catch(Throwable $e) {}

$ayar = $pdo->query("SELECT * FROM cdf_ayar LIMIT 1")->fetch(PDO::FETCH_ASSOC);
...
$fr_takimlar = [];
try { $fr_takimlar = $pdo->query("SELECT * FROM fr_takimlar ORDER BY puan DESC")->fetchAll(PDO::FETCH_ASSOC); } catch(Throwable $e) {}
...
if(isset($_POST['kupaya_basla'])) {
    try {
        $pdo->exec("DELETE FROM cdf_maclar WHERE sezon_yil = $sezon_yil");
        $pdo->exec("UPDATE cdf_ayar SET mevcut_tur='Tur 1', sampiyon=NULL WHERE id=1");
        $takimlar_ids = array_column($fr_takimlar, 'id');
        shuffle($takimlar_ids);
        // Eşit sayı için tek sayılı olanı sil
        if(count($takimlar_ids) % 2 != 0) array_pop($takimlar_ids);
        for($i=0; $i<count($takimlar_ids); $i+=2) {
            $pdo->exec("INSERT INTO cdf_maclar (ev, dep, tur, sezon_yil) VALUES ({$takimlar_ids[$i]}, {$takimlar_ids[$i+1]}, 'Tur 1', $sezon_yil)");
        }
        $mesaj = "Coupe de France başladı! Tur 1 kuraları çekildi."; $mesaj_tipi = "success";
        $mevcut_tur = 'Tur 1';
        header("Location: coupe_de_france.php"); exit;
    ...
    try {
        ...
        if($tur_idx !== false && $tur_idx < count($turler)-1) {
            ...
            if($sonraki_tur == 'Final' && count($kazananlar) == 2) {
                $pdo->exec("INSERT INTO cdf_maclar (ev, dep, tur, sezon_yil) VALUES ({$kazananlar[0]}, {$kazananlar[1]}, 'Final', $sezon_yil)");
            } elseif($sonraki_tur != 'Final') {
                shuffle($kazananlar);
                if(count($kazananlar) % 2 != 0) array_pop($kazananlar);
                for($i=0; $i<count($kazananlar); $i+=2) {
                    $pdo->exec("INSERT INTO cdf_maclar (ev, dep, tur, sezon_yil) VALUES ({$kazananlar[$i]}, {$kazananlar[$i+1]}, '$sonraki_tur', $sezon_yil)");
                }
            }
            $pdo->exec("UPDATE cdf_ayar SET mevcut_tur='$sonraki_tur' WHERE id=1");
            ...
            $sampiyon_takim = $pdo->query("SELECT takim_adi FROM fr_takimlar WHERE id={$kazananlar[0]}")->fetchColumn();
            $pdo->prepare("UPDATE cdf_ayar SET sampiyon=? WHERE id=1")->execute([$sampiyon_takim]);
            // Şampiyona ödül
            $pdo->exec("UPDATE fr_takimlar SET butce=butce+5000000 WHERE id={$kazananlar[0]}");
            $sampiyon = $sampiyon_takim;
            $mesaj = "COUPE DE FRANCE ŞAMPIYONU: $sampiyon_takim! 🏆 (€5M Ödül)"; $mesaj_tipi = "success";
        }
...
try {
    foreach($turler as $tur) {
        $maclar = $pdo->query("SELECT m.*, t1.takim_adi as ev_ad, t1.logo as ev_logo, t2.takim_adi as dep_ad, t2.logo as dep_logo FROM cdf_maclar m JOIN fr_takimlar t1 ON m.ev=t1.id JOIN fr_takimlar t2 ON m.dep=t2.id WHERE m.tur='$tur' AND m.sezon_yil=$sezon_yil ORDER BY m.id")->fetchAll(PDO::FETCH_ASSOC);
...
<head>
    ...
    <title>Coupe de France | Ultimate Manager</title>
    ...
    <style>
        ...
        body { background:var(--bg); color:var(--text); font-family:'Inter',sans-serif; min-height:100vh; background-image:radial-gradient(circle at 0% 0%,rgba(0,63,138,0.12) 0%,transparent 50%); }
        ...
        .match-row:hover { background:rgba(0,63,138,0.06); }
        ...
        .score-text { font-family:'Oswald',sans-serif; font-size:1.4rem; font-weight:900; color:var(--fr-gold); }
        .vs-text { color:var(--muted); font-size:0.85rem; font-weight:700; }
        .hero-cup { text-align:center; padding:40px 20px; }
        .hero-cup h1 { font-family:'Oswald',sans-serif; font-size:3rem; color:var(--fr-gold); }
        .champion-card { background:linear-gradient(135deg,rgba(0,63,138,0.4),rgba(239,65,53,0.2)); border:2px solid var(--fr-gold); border-radius:20px; padding:30px; text-align:center; max-width:400px; margin:0 auto 30px; }
        .btn-simulate { background:linear-gradient(135deg,var(--fr-primary),var(--fr-secondary)); color:#fff; border:none; border-radius:8px; padding:12px 24px; font-family:'Oswald',sans-serif; font-size:1rem; font-weight:800; cursor:pointer; transition:0.2s; }
        .btn-simulate:hover { transform:translateY(-2px); box-shadow:0 6px 20px rgba(0,63,138,0.4); }
        .tur-badge { display:inline-block; background:rgba(0,63,138,0.3); border:1px solid rgba(0,63,138,0.5); padding:4px 12px; border-radius:20px; font-size:0.75rem; font-weight:700; color:#60a5fa; text-transform:uppercase; }
    </style>
</head>
<body>
<nav class="pro-navbar">
    <a href="../ligue_1/ligue1.php" class="nav-brand font-oswald"><i class="fa-solid fa-shield-halved"></i> COUPE DE FRANCE</a>
    <div class="d-none d-lg-flex gap-2">
        <a href="../index.php" class="nav-link-item"><i class="fa-solid fa-house"></i> Merkez</a>
        <a href="../ligue_1/ligue1.php" class="nav-link-item"><i class="fa-solid fa-flag"></i> Ligue 1</a>
        <a href="../ligue_1/l1_kadro.php" class="nav-link-item"><i class="fa-solid fa-users"></i> Kadro</a>
        <a href="../ligue_1/l1_transfer.php" class="nav-link-item"><i class="fa-solid fa-comments-dollar"></i> Transfer</a>
    </div>
    <a href="../ligue_1/ligue1.php" class="btn-ap"><i class="fa-solid fa-arrow-left"></i> Ligue 1</a>
</nav>
...
<div class="container py-4" style="max-width:900px;">
    ...
    <div class="hero-cup">
        ...
        <h1>COUPE DE FRANCE <?=$sezon_yil?></h1>
        <p style="color:var(--muted);">Fransa'nın en prestijli ulusal kupası — Tüm Ligue 1 takımları katılır</p>
    </div>

    <?php if($sampiyon): ?>
    <div class="champion-card">
        <div style="font-size:3rem;margin-bottom:10px;">🏆</div>
        <div style="font-size:0.8rem;color:var(--muted);text-transform:uppercase;letter-spacing:2px;">Coupe de France Şampiyonu</div>
    ...
    <div class="panel-card">
        ...
        <div class="p-4 d-flex gap-3 flex-wrap">
            ...
            <form method="POST">
                <button type="submit" name="kupaya_basla" class="btn-simulate">
                    <i class="fa-solid fa-play me-2"></i>Coupe de France'ı Başlat
                </button>
            </form>
            <?php else: ?>
            <form method="POST">
        ...
        <div class="px-4 pb-3" style="font-size:0.85rem;color:var(--muted);">
            ...
            <?php if(empty($fr_takimlar)): ?>
            <span class="ms-3" style="color:#f87171;"><i class="fa-solid fa-exclamation-triangle"></i> Ligue 1 takımları bulunamadı. Önce Ligue 1'e gidin.</span>
            <?php endif; ?>
        </div>
    </div>
    ...
    <div class="panel-card">
        <div class="p-5 text-center" style="color:var(--muted);">
            <i class="fa-solid fa-trophy fa-3x mb-3" style="color:var(--fr-gold);"></i>
            <div class="font-oswald fs-3">Coupe de France Başlamadı</div>
