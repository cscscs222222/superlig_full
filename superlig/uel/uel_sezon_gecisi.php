<?php
// ==============================================================================
// UEFA EUROPA LEAGUE - SEZON SONU VE ŞAMPİYON KUTLAMASI
// ==============================================================================
include '../db.php';
...
if ($final_mac) {
    ...
    $sampiyon = $pdo->query("SELECT * FROM uel_takimlar WHERE id = $sampiyon_id")->fetch(PDO::FETCH_ASSOC);
    $finalist = $pdo->query("SELECT * FROM uel_takimlar WHERE id = $finalist_id")->fetch(PDO::FETCH_ASSOC);
...
if (empty($puan_durumu) || !$sampiyon) { header("Location: uel.php"); exit; }
...
if (isset($_POST['yeni_sezona_gec'])) {
    ...
    foreach ($puan_durumu as $index => $t) {
        ...
        $pdo->exec("UPDATE uel_takimlar SET butce = butce + $odul WHERE id = {$t['id']}");
    }
    ...
    if ($sampiyon) {
        try {
            $stmt = $pdo->prepare("INSERT INTO tournaments (turnuva, sezon_yil, sampiyon_id, sampiyon_adi, sampiyon_lig) VALUES ('UEL', ?, ?, ?, ?)
    ...
    $pdo->exec("UPDATE uel_oyuncular SET yas = yas + 1, form = 6, fitness = 100, ceza_hafta = 0, sakatlik_hafta = 0");
    $pdo->exec("DELETE FROM uel_oyuncular WHERE yas >= 38");
    ...
    $pdo->exec("TRUNCATE TABLE uel_takimlar");
    $pdo->exec("TRUNCATE TABLE uel_oyuncular");

    // 7. YILI İLERLET
    $yeni_sezon = $guncel_sezon + 1;
    $pdo->exec("UPDATE uel_ayar SET hafta = 1, sezon_yil = $yeni_sezon, kullanici_takim_id = NULL");
...
<head>
    ...
    <title>UEL Şampiyonu | Europa League</title>
    ...
    <style>
        :root { --uel-orange:#f04e23; --gold:#d4af37; --bg:#100501; }
        body { background:var(--bg); color:#fff; font-family:'Poppins',sans-serif; min-height:100vh; display:flex; align-items:center; justify-content:center;
            background-image:radial-gradient(circle at 50% 30%, rgba(240,78,35,0.15) 0%, transparent 70%); }
        .font-oswald { font-family:'Oswald',sans-serif; text-transform:uppercase; }
        .celebration-card { background:linear-gradient(135deg,rgba(30,16,8,0.95),rgba(0,0,0,0.95)); border:2px solid var(--uel-orange); border-radius:24px; padding:50px; text-align:center; max-width:750px; width:100%; box-shadow:0 0 60px rgba(240,78,35,0.25); }
        ...
        .btn-uel { background:linear-gradient(135deg,var(--uel-orange),#b83310); color:#fff; font-weight:900; font-size:1.1rem; padding:14px 36px; border:none; border-radius:50px; cursor:pointer; text-transform:uppercase; transition:0.3s; text-decoration:none; display:inline-flex; align-items:center; gap:10px; }
        .btn-uel:hover { box-shadow:0 0 30px rgba(240,78,35,0.5); transform:scale(1.05); color:#fff; }
        .season-summary { display:flex; justify-content:center; gap:24px; margin-top:30px; border-top:1px solid rgba(240,78,35,0.3); padding-top:28px; flex-wrap:wrap; }
        ...
        .summary-title { font-size:0.75rem; color:var(--uel-orange); font-weight:700; letter-spacing:1px; }
...
<div class="container">
    <div class="celebration-card mx-auto">
        <i class="fa-solid fa-fire mb-3" style="font-size:3.5rem;color:var(--uel-orange);"></i>
        <div class="text-muted fw-bold mb-2" style="letter-spacing:2px;font-size:0.85rem;"><?= $guncel_sezon ?>/<?= $guncel_sezon+1 ?> EUROPA LEAGUE ŞAMPIYONU</div>
        ...
        <?php if ($final_skor): ?>
        <div style="margin:16px auto;background:rgba(0,0,0,0.6);border:1px solid rgba(240,78,35,0.3);border-radius:12px;padding:14px 28px;display:inline-block;">
        ...
        <form method="POST" class="mt-4">
            <button type="submit" name="yeni_sezona_gec" class="btn-uel" onclick="return confirm('Tüm istatistikler sıfırlanacak. Sezon sona erecek. Emin misiniz?');">
