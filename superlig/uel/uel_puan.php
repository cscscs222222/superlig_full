<?php
// ==============================================================================
// UEFA EUROPA LEAGUE - PUAN DURUMU VE İSTATİSTİKLER
// ==============================================================================
include '../db.php';
...
$puan_durumu = $pdo->query("SELECT * FROM uel_takimlar ORDER BY puan DESC, (atilan_gol - yenilen_gol) DESC, atilan_gol DESC")->fetchAll(PDO::FETCH_ASSOC);

// İstatistikler: gol, asist, kart
$tum_maclar = [];
try { $tum_maclar = $pdo->query("SELECT ev_olaylar, dep_olaylar, ev_kartlar, dep_kartlar FROM uel_maclar WHERE ev_skor IS NOT NULL")->fetchAll(PDO::FETCH_ASSOC); } catch(Throwable $e){}
$goller = []; $asistler = [];
...
$takim_form = [];
$form_maclar = [];
try { $form_maclar = $pdo->query("SELECT ev, dep, ev_skor, dep_skor FROM uel_maclar WHERE ev_skor IS NOT NULL ORDER BY hafta DESC")->fetchAll(PDO::FETCH_ASSOC); } catch(Throwable $e){}
...
<head>
    ...
    <style>
        :root { --uel-orange:#f04e23; --uel-dark:#1a0a02; --uel-panel:#1e1008; --uel-border:rgba(240,78,35,0.2); --uel-accent:#ff6b35; --gold:#d4af37; }
        body { background:var(--uel-dark); color:#fff; font-family:'Poppins',sans-serif; min-height:100vh; }
        ...
        .panel-title { font-family:'Oswald',sans-serif; font-size:1.1rem; font-weight:700; color:var(--uel-accent); letter-spacing:2px; text-transform:uppercase; margin-bottom:16px; padding-bottom:12px; border-bottom:1px solid var(--uel-border); }
        .puan-tbl { width:100%; border-collapse:collapse; }
        .puan-tbl th { font-size:0.75rem; color:#666; letter-spacing:1px; text-transform:uppercase; padding:8px 10px; border-bottom:1px solid var(--uel-border); text-align:center; }
        ...
        .puan-tbl tr.kullanici-satir td { background:rgba(240,78,35,0.12); border-left:3px solid var(--uel-orange); }
        ...
        .sira-ko { background:rgba(240,78,35,0.2); color:var(--uel-accent); border:1px solid rgba(240,78,35,0.4); }
        ...
        .stat-pos { font-weight:700; color:var(--uel-accent); width:20px; }
        .stat-name { flex:1; }
...
<nav class="pro-navbar">
    <a href="uel.php" class="nav-brand font-oswald"><i class="fa-solid fa-fire" style="color:var(--uel-accent);"></i>&nbsp;EUROPA LEAGUE</a>
    <div class="nav-links d-flex gap-2">
        <a href="uel.php"><i class="fa-solid fa-calendar-days"></i> Fikstür</a>
        <a href="uel_puan.php" class="active"><i class="fa-solid fa-table"></i> Puan Durumu</a>
...
<div class="container-fluid py-4 px-4">
    <h2 class="font-oswald mb-4" style="color:var(--uel-accent);font-size:2rem;"><i class="fa-solid fa-ranking-star me-2"></i>EUROPA LEAGUE PUAN DURUMU — <?= $sezon_yili ?>/<?= $sezon_yili+1 ?></h2>
    ...
    <div class="row g-4">
        <div class="col-lg-8">
            <div class="panel">
                ...
                <div class="legend">
                    <div class="legend-item"><div class="legend-dot" style="background:var(--uel-accent);"></div><span style="color:#999;">Son 16 (1-16. sıra)</span></div>
                        ...
                        <tr class="<?= ($kullanici_takim_id == $t['id']) ? 'kullanici-satir' : '' ?>">
                            ...
                            <td>
                                <div class="d-flex align-items-center gap-2">
                                    ...
                                    <span class="badge" style="background:rgba(240,78,35,0.2);color:var(--uel-accent);font-size:0.65rem;">SEN</span>
                                    <?php endif; ?>
