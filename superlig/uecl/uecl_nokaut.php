<?php
// ==============================================================================
// CONFERENCE LEAGUE - NOKAUT AŞAMASI V5.0 (İLK 8 DİREKT SON 16 MANTIĞI)
// ==============================================================================
include '../db.php';
...
function mac_var_mi($pdo, $h, $sezon_yil) {
    ...
    return $pdo->query("SELECT COUNT(*) FROM uecl_maclar WHERE hafta = $h AND sezon_yil = $sezon_yil")->fetchColumn() > 0;
}

function get_kazananlar($pdo, $h1, $h2, $sezon_yil) {
    $kazananlar = [];
    $h1 = (int)$h1; $h2 = (int)$h2; $sezon_yil = (int)$sezon_yil;
    $maclar1 = $pdo->query("SELECT * FROM uecl_maclar WHERE hafta=$h1 AND sezon_yil=$sezon_yil")->fetchAll(PDO::FETCH_ASSOC);
    foreach($maclar1 as $m1) {
        $ev = (int)$m1['ev']; $dep = (int)$m1['dep'];
        $m2 = $pdo->query("SELECT * FROM uecl_maclar WHERE hafta=$h2 AND sezon_yil=$sezon_yil AND ev=$dep AND dep=$ev")->fetch(PDO::FETCH_ASSOC);
...
if ($hafta == 9 && !mac_var_mi($pdo, 9, $sezon_yili)) {
    ...
    $takimlar = array_map('intval', $pdo->query("SELECT id FROM uecl_takimlar ORDER BY puan DESC, (atilan_gol - yenilen_gol) DESC, atilan_gol DESC LIMIT 8, 16")->fetchAll(PDO::FETCH_COLUMN));
    $n = count($takimlar);
    if($n == 16) {
        $yari = $n / 2;
        for($i=0; $i<$yari; $i++) {
            ...
            $pdo->exec("INSERT INTO uecl_maclar (ev, dep, hafta, sezon_yil) VALUES ($unseeded, $seeded, 9, $sezon_yili)");
            // Rövanş (Hafta 10): Yüksek sıralı (seeded) takımın evinde
            $pdo->exec("INSERT INTO uecl_maclar (ev, dep, hafta, sezon_yil) VALUES ($seeded, $unseeded, 10, $sezon_yili)");
        }
    }
}
if ($hafta == 11 && !mac_var_mi($pdo, 11, $sezon_yili)) {
    // Hafta 11-12: SON 16 – İlk 8 takım sahneye çıkar, play-off kazananlarıyla eşleşir
    $ilk8 = array_map('intval', $pdo->query("SELECT id FROM uecl_takimlar ORDER BY puan DESC, (atilan_gol - yenilen_gol) DESC, atilan_gol DESC LIMIT 8")->fetchAll(PDO::FETCH_COLUMN));
    $playoff = get_kazananlar($pdo, 9, 10, $sezon_yili);
    if(count($playoff) == 8 && count($ilk8) == 8) {
        for($i=0; $i<8; $i++) {
            $ev = (int)$ilk8[$i]; $dep = (int)$playoff[$i];
            $pdo->exec("INSERT INTO uecl_maclar (ev, dep, hafta, sezon_yil) VALUES ($ev, $dep, 11, $sezon_yili)");
            $pdo->exec("INSERT INTO uecl_maclar (ev, dep, hafta, sezon_yil) VALUES ($dep, $ev, 12, $sezon_yili)");
        }
    }
}
...
if ($hafta == 13 && !mac_var_mi($pdo, 13, $sezon_yili)) {
    ...
    if(count($c) == 8) {
        for($i=0; $i<4; $i++) {
            ...
            $pdo->exec("INSERT INTO uecl_maclar (ev, dep, hafta, sezon_yil) VALUES ($ev, $dep, 13, $sezon_yili)");
            $pdo->exec("INSERT INTO uecl_maclar (ev, dep, hafta, sezon_yil) VALUES ($dep, $ev, 14, $sezon_yili)");
        }
    }
}
...
if ($hafta == 15 && !mac_var_mi($pdo, 15, $sezon_yili)) {
    ...
    if(count($y) == 4) {
        for($i=0; $i<2; $i++) {
            ...
            $pdo->exec("INSERT INTO uecl_maclar (ev, dep, hafta, sezon_yil) VALUES ($ev, $dep, 15, $sezon_yili)");
            $pdo->exec("INSERT INTO uecl_maclar (ev, dep, hafta, sezon_yil) VALUES ($dep, $ev, 16, $sezon_yili)");
        }
    }
}
...
if ($hafta == 17 && !mac_var_mi($pdo, 17, $sezon_yili)) {
    ...
    if(count($f) == 2) {
        ...
        $pdo->exec("INSERT INTO uecl_maclar (ev, dep, hafta, sezon_yil) VALUES ($ev, $dep, 17, $sezon_yili)");
    }
}

// SİMÜLASYON İŞLEMİ
if(isset($_GET['simule'])) {
    $maclar = $pdo->query("SELECT * FROM uecl_maclar WHERE hafta = $hafta AND sezon_yil = $sezon_yili AND ev_skor IS NULL")->fetchAll(PDO::FETCH_ASSOC);
    foreach($maclar as $m) {
...
$maclar_ui = $pdo->query("SELECT m.*, t1.takim_adi as ev_ad, t1.logo as ev_logo, t2.takim_adi as dep_ad, t2.logo as dep_logo FROM uecl_maclar m JOIN uecl_takimlar t1 ON m.ev = t1.id JOIN uecl_takimlar t2 ON m.dep = t2.id WHERE m.hafta IN ($hedef_h) AND m.sezon_yil = $sezon_yili ORDER BY m.hafta ASC")->fetchAll(PDO::FETCH_ASSOC);

$benim_macim_var_mi = $pdo->query("SELECT COUNT(*) FROM uecl_maclar WHERE hafta=$hafta AND sezon_yil=$sezon_yili AND ev_skor IS NULL AND (ev=$kullanici_takim_id OR dep=$kullanici_takim_id)")->fetchColumn();
...
if ($hafta > 17) {
    ...
    if ($final_mac) {
        ...
        $stmt = $pdo->prepare("SELECT * FROM uecl_takimlar WHERE id = ?");
        $stmt->execute([$s_id]);
...
<head>
    <meta charset="UTF-8">
    <title>UECL Nokaut Aşaması</title>
...
<body>
    ...
    <div class="hero-banner">
        ...
        <div class="mt-3 d-flex justify-content-center gap-3 flex-wrap">
            <?php if($hafta > 17): ?>
                <a href="uecl_sezon_gecisi.php" class="btn btn-warning fw-bold font-oswald text-dark px-4 py-3" style="font-size: 1.1rem; box-shadow: 0 0 20px rgba(255,193,7,0.5); border-radius:8px;">
    ...
    <div style="max-width:860px; margin: 0 auto 30px; padding: 0 1rem;">
        <div style="background: linear-gradient(135deg, rgba(212,175,55,0.15), rgba(0,229,255,0.08)); border: 2px solid #d4af37; border-radius: 18px; padding: 36px 30px; text-align:center; box-shadow: 0 0 40px rgba(212,175,55,0.25);">
            <div style="font-family:'Oswald',sans-serif; font-size:0.9rem; color:#94a3b8; letter-spacing:3px; margin-bottom:12px;">UEFA KONFERANS LİGİ FİNALİ</div>
