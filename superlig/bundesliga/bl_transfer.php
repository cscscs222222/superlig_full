...
$benim_takim = $pdo->query("SELECT * FROM de_takimlar WHERE id = $kullanici_takim_id")->fetch(PDO::FETCH_ASSOC);
...
if (isset($_POST['satin_al'])) {
    ...
    $hedef_oyuncu = $pdo->query("SELECT * FROM de_oyuncular WHERE id = $oyuncu_id")->fetch(PDO::FETCH_ASSOC);
    ...
    if ($hedef_oyuncu) {
        ...
        if ($benim_takim['butce'] >= $fiyat) {
            try {
                ...
                $stmt_upd1 = $pdo->prepare("UPDATE de_takimlar SET butce = butce - ? WHERE id = ?");
                $stmt_upd1->execute([$fiyat, $kullanici_takim_id]);
                $stmt_upd2 = $pdo->prepare("UPDATE de_takimlar SET butce = butce + ? WHERE id = ?");
                $stmt_upd2->execute([$fiyat, $eski_takim_id]);
                $stmt_upd3 = $pdo->prepare("UPDATE de_oyuncular SET takim_id = ?, ilk_11 = 0, yedek = 1 WHERE id = ?");
            ...
            $mesaj = "Bütçe yetersiz! Bu transfer için Bundesliga kasanızda yeterli para yok.";
            $mesaj_tipi = "danger";
        }
    }
}
...
if (isset($_POST['sat'])) {
    $oyuncu_id = (int)$_POST['oyuncu_id'];
    $benim_oyuncu = $pdo->query("SELECT * FROM de_oyuncular WHERE id = $oyuncu_id AND takim_id = $kullanici_takim_id")->fetch(PDO::FETCH_ASSOC);
    
    if ($benim_oyuncu) {
        $fiyat = $benim_oyuncu['fiyat'];
        
        $ai_takim_stmt = $pdo->prepare("SELECT id FROM de_takimlar WHERE id != ? ORDER BY RAND() LIMIT 1");
        ...
        try {
            $pdo->beginTransaction();
            $stmt_s1 = $pdo->prepare("UPDATE de_takimlar SET butce = butce + ? WHERE id = ?");
            $stmt_s1->execute([$fiyat, $kullanici_takim_id]);
            $stmt_s2 = $pdo->prepare("UPDATE de_takimlar SET butce = GREATEST(0, butce - ?) WHERE id = ?");
            $stmt_s2->execute([$fiyat, $yeni_takim_id]);
            $stmt_s3 = $pdo->prepare("UPDATE de_oyuncular SET takim_id = ?, ilk_11 = 0, yedek = 1 WHERE id = ?");
...
$sql_pazar = "SELECT o.*, t.takim_adi, t.logo FROM de_oyuncular o JOIN de_takimlar t ON o.takim_id = t.id WHERE o.takim_id != :kid";
...
$benim_oyuncularim = $pdo->prepare("SELECT * FROM de_oyuncular WHERE takim_id = ? ORDER BY ovr DESC");
...
<head>
    ...
    <title>Transfer Borsası | Bundesliga</title>
    ...
    <style>
        ...
        .budget-card {
            ...
            border: 1px solid var(--de-secondary); border-radius: 12px; 
            padding: 1.5rem 3rem; display: inline-block; box-shadow: 0 10px 30px rgba(220,38,38,0.2);
        }
        ...
        .nav-pills .nav-link.active { background: var(--de-secondary); color: var(--de-dark); border-color: var(--de-secondary); box-shadow: 0 4px 15px rgba(220,38,38,0.4);}
        ...
        .btn-buy { background: var(--de-secondary); color: var(--de-dark); font-weight: 800; border: none; border-radius: 6px; padding: 6px 15px; font-size: 0.85rem; transition: 0.2s;}
        .btn-buy:hover { background: #ef4444; color: #000; box-shadow: 0 0 10px var(--de-secondary); transform: scale(1.05);}
        
        .btn-sell { background: transparent; border: 1px solid var(--de-primary); color: var(--de-primary); font-weight: 700; border-radius: 6px; padding: 6px 15px; font-size: 0.85rem; transition: 0.2s;}
        .btn-sell:hover { background: var(--de-primary); color: #fff; transform: scale(1.05); box-shadow: 0 0 10px rgba(200,16,46,0.5);}
        ...
        .filter-input:focus { outline: none; border-color: var(--de-secondary); background: rgba(220,38,38,0.05); }
        ...
        .filter-select:focus { outline: none; border-color: var(--de-secondary); }
        .filter-select option { background: var(--bg-panel); color: #fff; }
        .btn-search { background: var(--de-secondary); color: var(--de-dark); border: none; font-weight: 800; padding: 0 20px; border-radius: 6px; transition: 0.2s;}
        .btn-search:hover { background: #ef4444; color: #000;}
    </style>
</head>
<body>
    ...
    <nav class="pro-navbar">
        ...
        <div class="nav-menu d-none d-lg-flex gap-3">
            ...
            <a href="bl_transfer.php" class="nav-link-item text-white fw-bold" style="text-shadow: 0 0 10px var(--de-secondary);"><i class="fa-solid fa-comments-dollar"></i> Transfer</a>
        </div>

        <div class="d-flex gap-3">
            <a href="bundesliga.php" class="nav-link-item" style="border: 1px solid var(--de-primary); border-radius: 4px; color: var(--de-primary);">
    ...
    <div class="hero-banner">
        <div class="budget-card">
            <div style="font-size: 0.85rem; color:var(--text-muted); font-weight:700; letter-spacing:2px; margin-bottom:5px;" class="text-uppercase"><i class="fa-solid fa-vault" style="color:var(--de-secondary);"></i> Transfer Bütçesi</div>
            <h1 class="font-oswald m-0" style="font-size: 3rem; color: var(--de-secondary); text-shadow: 0 0 15px rgba(220,38,38,0.5);"><?= paraFormatla($benim_takim['butce']) ?></h1>
        </div>
    </div>
    ...
    <div class="container py-5" style="max-width: 1400px;">
        ...
        <ul class="nav nav-pills justify-content-center mb-4 gap-3" id="transferTabs">
            <li class="nav-item">
                <button class="nav-link active font-oswald fs-5" data-bs-toggle="pill" data-bs-target="#pazar"><i class="fa-solid fa-globe"></i> Bundesliga Pazarı</button>
            </li>
            <li class="nav-item">
            ...
            <div class="tab-pane fade show active" id="pazar">
                <div class="panel-card shadow-lg">
                    <form method="GET" class="filter-bar">
                        <input type="text" name="q" class="filter-input" placeholder="Bundesliga yıldızlarını veya takım adını ara..." value="<?= htmlspecialchars($arama) ?>">
                        ...
                        <table class="data-table">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td style="color: var(--de-secondary); font-weight: 700;"><?= $o['form'] ?>.0</td>
                                    <td class="text-end font-oswald fs-5" style="color: var(--de-secondary); letter-spacing:0.5px;"><?= paraFormatla($o['fiyat']) ?></td>
                                ...
                                <?php if(empty($pazardaki_oyuncular)): ?>
                                    <tr><td colspan="8" class="text-center py-5 text-muted font-oswald fs-5"><i class="fa-solid fa-ghost fs-3 mb-2 d-block opacity-50"></i> Bundesliga pazarında aradığınız kriterlerde oyuncu bulunamadı.</td></tr>
                                <?php endif; ?>
                            </tbody>
                        </table>
                    </div>
                ...
                <div class="panel-card shadow-lg" style="border-color: rgba(200,16,46,0.5);">
                    <div class="table-responsive" style="max-height: 650px; overflow-y: auto;">
                        <table class="data-table">
                            <thead style="position: sticky; top: 0; z-index: 10;">
                                <tr>
                                    <th width="5%" style="color:var(--de-primary);">Pos</th>
                                    <th style="color:var(--de-primary);">Oyuncu İsmi</th>
                                    <th style="color:var(--de-primary);">Kadro Durumu</th>
                                    <th style="color:var(--de-primary);">Yaş</th>
                                    <th style="color:var(--de-primary);">OVR</th>
                                    <th class="text-end" style="color:var(--de-primary);">Beklenen Gelir</th>
                                    <th class="text-center" style="color:var(--de-primary);">İşlem</th>
                                </tr>
                            </thead>
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="fw-bold text-start text-white"><?= htmlspecialchars($o['isim']) ?></td>
                                    <td>
                                        <?php if($o['ilk_11']==1): ?><span class="badge" style="background:var(--de-secondary); color:var(--de-dark);">İlk 11</span>
                                    ...
                                    <td class="text-end font-oswald fs-5" style="color: var(--de-secondary);"><?= paraFormatla($o['fiyat']) ?></td>
                                    <td class="text-center">
                                        <form method="POST">
                                            <input type="hidden" name="oyuncu_id" value="<?= $o['id'] ?>">
    ...
    <div class="d-flex d-lg-none fixed-bottom p-2 justify-content-around align-items-center border-top" style="background: rgba(13,13,13,0.95); backdrop-filter: blur(10px); z-index:2000; padding-bottom: 15px !important; border-top-color: var(--de-secondary) !important;">
        ...
        <a href="bl_kadro.php" class="text-secondary text-decoration-none text-center" style="font-size: 0.8rem; width: 33%;">
            <i class="fa-solid fa-users fs-5 mb-1 d-block text-white"></i> Kadro
        </a>
        <a href="bl_transfer.php" class="text-decoration-none text-center fw-bold" style="font-size: 0.8rem; width: 33%; color: var(--de-secondary);">
