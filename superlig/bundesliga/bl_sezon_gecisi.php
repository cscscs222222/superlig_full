...
if(isset($_POST['yeni_sezona_gec'])) {
    ...
    // 1. ÖDÜL DAĞITIMI (BUNDESLIGA YAYIN GELİRLERİ)
    foreach($puan_durumu as $index => $t) {
        ...
        $pdo->exec("UPDATE de_takimlar SET butce = butce + $odul WHERE id = {$t['id']}");
    }

    // 2. ŞAMPİYONLAR LİGİ'NE İHRAÇ (İLK 4 TAKIM ALMANYA'DAN GİDER)
    ...
    foreach($cl_gidenler as $cl_takim) {
        ...
        if(!$var_mi) {
            $stmt = $pdo->prepare("INSERT INTO cl_takimlar (takim_adi, logo, hucum, savunma, butce, lig) VALUES (?, ?, ?, ?, ?, 'Bundesliga')");
            $stmt->execute([$cl_takim['takim_adi'], $cl_takim['logo'], $cl_takim['hucum'], $cl_takim['savunma'], 50000000]);
            $yeni_cl_id = $pdo->lastInsertId();
            
            $oyuncular = $pdo->query("SELECT * FROM de_oyuncular WHERE takim_id = {$cl_takim['id']}")->fetchAll(PDO::FETCH_ASSOC);
            foreach($oyuncular as $o) {
                $stmt_o = $pdo->prepare("INSERT INTO cl_oyuncular (takim_id, isim, mevki, ovr, yas, fiyat, lig, ilk_11, yedek) VALUES (?, ?, ?, ?, ?, ?, 'Bundesliga', ?, ?)");
...
<head>
    ...
    <title>Sezon Sonu | Bundesliga</title>
    ...
    <style>
        ...
        body { 
            ...
            background-image: radial-gradient(circle at 50% 30%, rgba(220, 38, 38, 0.15) 0%, transparent 70%);
            min-height: 100vh; display: flex; align-items: center; justify-content: center;
        }
        .font-oswald { font-family: 'Oswald', sans-serif; text-transform: uppercase; }

        .celebration-card {
            background: rgba(26, 26, 26, 0.9); border: 2px solid var(--de-secondary);
            border-radius: 20px; padding: 50px; text-align: center; box-shadow: 0 0 50px rgba(220, 38, 38, 0.2);
            max-width: 800px; width: 100%; position: relative; overflow: hidden;
        }
        .celebration-card::before {
            content: ''; position: absolute; top: 0; left: 0; right: 0; height: 4px;
            background: linear-gradient(90deg, var(--de-primary), var(--de-secondary), var(--de-primary));
        }

        .champ-logo { width: 150px; height: 150px; object-fit: contain; filter: drop-shadow(0 0 20px var(--de-secondary)); margin-bottom: 20px; animation: float 3s ease-in-out infinite;}
        @keyframes float { 0% { transform: translateY(0px); } 50% { transform: translateY(-10px); } 100% { transform: translateY(0px); } }

        .btn-es { background: var(--de-secondary); color: var(--de-primary); font-weight: 900; font-size: 1.2rem; padding: 15px 40px; border: none; border-radius: 50px; margin-top: 30px; transition: 0.3s; text-transform: uppercase;}
        .btn-es:hover { box-shadow: 0 0 30px var(--de-secondary); transform: scale(1.05); color: #000;}

        .season-summary { display: flex; justify-content: center; gap: 30px; margin-top: 30px; border-top: 1px solid rgba(220,38,38,0.3); padding-top: 30px; flex-wrap: wrap;}
        .summary-item { background: rgba(0,0,0,0.5); padding: 15px 25px; border-radius: 10px; border: 1px solid rgba(255,255,255,0.1);}
        .summary-title { font-size: 0.8rem; color: var(--de-primary); font-weight: 700; letter-spacing: 1px;}
        .summary-val { font-family: 'Oswald'; font-size: 1.5rem; color: var(--de-secondary);}
    </style>
</head>
<body>
    <div class="container">
        <div class="celebration-card mx-auto">
            ...
            <div class="text-muted fw-bold mb-2" style="color: var(--de-secondary) !important;"><?= $guncel_sezon ?> - <?= $guncel_sezon + 1 ?> BUNDESLIGA ŞAMPİYONU</div>
            <img src="<?= htmlspecialchars($sampiyon['logo']) ?>" class="champ-logo" alt="Şampiyon">
            <h1 class="font-oswald" style="font-size: 4rem; color: var(--de-secondary);"><?= htmlspecialchars($sampiyon['takim_adi']) ?></h1>
            <p class="fs-5 mt-3 text-light">Meister! <?= $sampiyon['puan'] ?> Puan ile Bundesliga zirvesine ulaştı!</p>
            ...
            <form method="POST">
                <button type="submit" name="yeni_sezona_gec" class="btn-es" onclick="return confirm('Tüm istatistikler sıfırlanacak ve yeni Bundesliga sezonu başlayacak. Bestätigen?');">
                    <i class="fa-solid fa-forward"></i> Neue Saison (Yeni Sezon)
                </button>
            </form>
        </div>
    </div>
