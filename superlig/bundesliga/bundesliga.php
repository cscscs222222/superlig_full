...
// Merkez Maç Motorunu Bağla (ÖN EK OLARAK 'de_' VERİYORUZ!)
if(file_exists('../MatchEngine.php')) {
    include '../MatchEngine.php';
    $engine = new MatchEngine($pdo, 'de_');
} else {
...
sutunEkle($pdo, 'de_takimlar', 'lig', "VARCHAR(50) DEFAULT 'Bundesliga'");
...
if ($de_takim_sayisi < 18) {
    ...
    ['Bayer 04 Leverkusen',     'https://tmssl.akamaized.net/images/wappen/head/15.png',    84, 82],
    ['RB Leipzig',              'https://tmssl.akamaized.net/images/wappen/head/23826.png', 83, 82],
    ['Borussia Mönchengladbach','https://tmssl.akamaized.net/images/wappen/head/23.png',    79, 78],
    ['Eintracht Frankfurt',     'https://tmssl.akamaized.net/images/wappen/head/24.png',    79, 78],
    ['VfL Wolfsburg',           'https://tmssl.akamaized.net/images/wappen/head/82.png',    78, 77],
    ['SC Freiburg',             'https://tmssl.akamaized.net/images/wappen/head/17.png',    77, 77],
    ['Union Berlin',            'https://tmssl.akamaized.net/images/wappen/head/89.png',    76, 76],
    ['VfB Stuttgart',           'https://tmssl.akamaized.net/images/wappen/head/79.png',    76, 75],
    ['FC Augsburg',             'https://tmssl.akamaized.net/images/wappen/head/167.png',   74, 74],
    ['Werder Bremen',           'https://tmssl.akamaized.net/images/wappen/head/86.png',    74, 74],
    ['Hoffenheim',              'https://tmssl.akamaized.net/images/wappen/head/533.png',   73, 73],
    ['Mainz 05',                'https://tmssl.akamaized.net/images/wappen/head/39.png',    73, 73],
    ['Heidenheim',              'https://tmssl.akamaized.net/images/wappen/head/2036.png',  72, 72],
    ['Hamburger SV',            'https://tmssl.akamaized.net/images/wappen/head/41.png',    72, 72],
    ['Bochum',                  'https://tmssl.akamaized.net/images/wappen/head/80.png',    71, 71],
    ['Köln',                    'https://tmssl.akamaized.net/images/wappen/head/3.png',     71, 71],
];
    
    foreach($almanya_devleri as $d) {
        $ad = $d[0]; $logo = $d[1]; $huc = $d[2]; $sav = $d[3];
        $var_mi = $pdo->query("SELECT COUNT(*) FROM de_takimlar WHERE takim_adi = " . $pdo->quote($ad))->fetchColumn();
        if($var_mi == 0) {
            $butce = rand(20000000, 200000000);
            $stmt = $pdo->prepare("INSERT INTO de_takimlar (takim_adi, logo, hucum, savunma, butce, lig) VALUES (?, ?, ?, ?, ?, 'Bundesliga')");
            ...
            for($i=0; $i<22; $i++) {
                ...
                $yedek = ($i >= 11 && $i < 18) ? 1 : 0;
                $fiyat = ($ovr * $ovr) * 4000;
                $stmt2 = $pdo->prepare("INSERT INTO de_oyuncular (takim_id, isim, mevki, ovr, yas, fiyat, lig, ilk_11, yedek) VALUES (?, ?, ?, ?, ?, ?, 'Bundesliga', ?, ?)");
...
$takimlar_listesi = $pdo->query("SELECT id, takim_adi, hucum, savunma FROM de_takimlar")->fetchAll(PDO::FETCH_ASSOC);
foreach($takimlar_listesi as $t) {
    $oyuncu_sayisi = $pdo->query("SELECT COUNT(*) FROM de_oyuncular WHERE takim_id = {$t['id']}")->fetchColumn();
    if($oyuncu_sayisi == 0) {
        for($i=0; $i<22; $i++) {
            ...
            $yedek = ($i >= 11 && $i < 18) ? 1 : 0;
            $fiyat = ($ovr * $ovr) * 4000;
            $stmt = $pdo->prepare("INSERT INTO de_oyuncular (takim_id, isim, mevki, ovr, yas, fiyat, lig, ilk_11, yedek) VALUES (?, ?, ?, ?, ?, ?, 'Bundesliga', ?, ?)");
...
function garanti_olay_uret($pdo, $takim_id, $skor) {
    $oyuncular = $pdo->query("SELECT isim FROM de_oyuncular WHERE takim_id = $takim_id AND ilk_11 = 1")->fetchAll(PDO::FETCH_COLUMN);
    if(empty($oyuncular)) $oyuncular = $pdo->query("SELECT isim FROM de_oyuncular WHERE takim_id = $takim_id")->fetchAll(PDO::FETCH_COLUMN);
...
$max_hafta = 34; // Bundesliga 18 takım, 34 haftadır.

// --- FİKSTÜR OLUŞTURMA ---
$mac_sayisi = 0;
try { $mac_sayisi = $pdo->query("SELECT COUNT(*) FROM de_maclar WHERE sezon_yil = $sezon_yili")->fetchColumn(); } catch(Throwable $e){}

if($mac_sayisi == 0) {
    $takimlar = $pdo->query("SELECT id FROM de_takimlar ORDER BY RAND()")->fetchAll(PDO::FETCH_COLUMN);
    if(count($takimlar) > 1) {
        ...
        for ($h = 1; $h <= $yari; $h++) {
            for ($i = 0; $i < $m_sayisi; $i++) {
                ...
                if ($ev != 0 && $dep != 0) {
                    if ($i % 2 == 0) { 
                        $pdo->exec("INSERT INTO de_maclar (ev, dep, hafta, sezon_yil) VALUES ($ev, $dep, $h, $sezon_yili)"); 
                        $pdo->exec("INSERT INTO de_maclar (ev, dep, hafta, sezon_yil) VALUES ($dep, $ev, ".($h+$yari).", $sezon_yili)"); 
                    } else { 
                        $pdo->exec("INSERT INTO de_maclar (ev, dep, hafta, sezon_yil) VALUES ($dep, $ev, $h, $sezon_yili)"); 
                        $pdo->exec("INSERT INTO de_maclar (ev, dep, hafta, sezon_yil) VALUES ($ev, $dep, ".($h+$yari).", $sezon_yili)"); 
                    }
                }
            }
            $son = array_pop($takimlar); array_splice($takimlar, 1, 0, [$son]);
        }
        $pdo->exec("INSERT INTO de_haberler (hafta, metin, tip) VALUES (1, 'Bundesliga Başlıyor! Almanya''da nefesler tutuldu.', 'sistem')");
        header("Location: bundesliga.php"); exit;
    }
}
...
if(isset($_GET['action'])) {
    ...
    if($action == 'tek_mac_simule' && isset($_GET['mac_id'])) {
        ...
        $m = $pdo->query("SELECT m.id, m.ev, m.dep, t1.takim_adi as ev_ad, t2.takim_adi as dep_ad, t1.hucum as ev_hucum, t1.savunma as ev_savunma, t2.hucum as dep_hucum, t2.savunma as dep_savunma 
                           FROM de_maclar m JOIN de_takimlar t1 ON m.ev=t1.id JOIN de_takimlar t2 ON m.dep=t2.id 
        ...
        if($m) {
            ...
            $stmt = $pdo->prepare("UPDATE de_maclar SET ev_skor=?, dep_skor=?, ev_olaylar=?, dep_olaylar=?, ev_kartlar=?, dep_kartlar=? WHERE id=?");
            $stmt->execute([$ev_skor, $dep_skor, $ev_detay['olaylar'], $dep_detay['olaylar'], $ev_detay['kartlar'], $dep_detay['kartlar'], $m['id']]);
            
            $pdo->exec("UPDATE de_takimlar SET atilan_gol = atilan_gol + $ev_skor, yenilen_gol = yenilen_gol + $dep_skor WHERE id = {$m['ev']}");
            $pdo->exec("UPDATE de_takimlar SET atilan_gol = atilan_gol + $dep_skor, yenilen_gol = yenilen_gol + $ev_skor WHERE id = {$m['dep']}");
            
            if($ev_skor > $dep_skor) { $pdo->exec("UPDATE de_takimlar SET puan=puan+3, galibiyet=galibiyet+1 WHERE id={$m['ev']}"); $pdo->exec("UPDATE de_takimlar SET malubiyet=malubiyet+1 WHERE id={$m['dep']}"); }
            elseif($ev_skor == $dep_skor) { $pdo->exec("UPDATE de_takimlar SET puan=puan+1, beraberlik=beraberlik+1 WHERE id IN ({$m['ev']}, {$m['dep']})"); }
            else { $pdo->exec("UPDATE de_takimlar SET puan=puan+3, galibiyet=galibiyet+1 WHERE id={$m['dep']}"); $pdo->exec("UPDATE de_takimlar SET malubiyet=malubiyet+1 WHERE id={$m['ev']}"); }
        }
        
        $kalan_mac = $pdo->query("SELECT COUNT(*) FROM de_maclar WHERE hafta = $hafta AND ev_skor IS NULL")->fetchColumn();
        if($kalan_mac == 0) { 
            $pdo->exec("UPDATE de_ayar SET hafta = LEAST($max_hafta, hafta + 1)"); 
        }
        header("Location: bundesliga.php?hafta=$hedef_hafta"); exit;
    }
    ...
    if($action == 'sezonu_simule') {
        for($h = $hafta; $h <= $max_hafta; $h++) {
            $maclar_h = $pdo->query("SELECT m.id, m.ev, m.dep, t1.hucum as ev_hucum, t1.savunma as ev_savunma, t2.hucum as dep_hucum, t2.savunma as dep_savunma FROM de_maclar m JOIN de_takimlar t1 ON m.ev=t1.id JOIN de_takimlar t2 ON m.dep=t2.id WHERE m.hafta=$h AND m.ev_skor IS NULL")->fetchAll(PDO::FETCH_ASSOC);
            foreach($maclar_h as $m) {
                $skorlar = $engine->gercekci_skor_hesapla($m['ev'], $m['dep'], $m);
                $ev_skor = $skorlar['ev']; $dep_skor = $skorlar['dep'];
                $ev_detay = $engine->mac_olay_uret($m['ev'], $ev_skor);
                $dep_detay = $engine->mac_olay_uret($m['dep'], $dep_skor);
                $pdo->prepare("UPDATE de_maclar SET ev_skor=?,dep_skor=?,ev_olaylar=?,dep_olaylar=?,ev_kartlar=?,dep_kartlar=? WHERE id=?")->execute([$ev_skor,$dep_skor,$ev_detay['olaylar'],$dep_detay['olaylar'],$ev_detay['kartlar'],$dep_detay['kartlar'],$m['id']]);
                $pdo->exec("UPDATE de_takimlar SET atilan_gol=atilan_gol+$ev_skor,yenilen_gol=yenilen_gol+$dep_skor WHERE id={$m['ev']}");
                $pdo->exec("UPDATE de_takimlar SET atilan_gol=atilan_gol+$dep_skor,yenilen_gol=yenilen_gol+$ev_skor WHERE id={$m['dep']}");
                if($ev_skor>$dep_skor){$pdo->exec("UPDATE de_takimlar SET puan=puan+3,galibiyet=galibiyet+1 WHERE id={$m['ev']}");$pdo->exec("UPDATE de_takimlar SET malubiyet=malubiyet+1 WHERE id={$m['dep']}");}
                elseif($ev_skor==$dep_skor){$pdo->exec("UPDATE de_takimlar SET puan=puan+1,beraberlik=beraberlik+1 WHERE id IN ({$m['ev']},{$m['dep']})");}
        ...
        $pdo->exec("UPDATE de_ayar SET hafta=$max_hafta");
        header("Location: bundesliga.php?sezon_bitti=1"); exit;
    }
}
...
$haftanin_fiksturu = $pdo->query("SELECT m.*, t1.takim_adi as ev_ad, t1.logo as ev_logo, t2.takim_adi as dep_ad, t2.logo as dep_logo FROM de_maclar m JOIN de_takimlar t1 ON m.ev = t1.id JOIN de_takimlar t2 ON m.dep = t2.id WHERE m.hafta = $goster_hafta")->fetchAll(PDO::FETCH_ASSOC);
$yayinlanacak_maclar = array_filter($haftanin_fiksturu, function($m) { return $m['ev_skor'] === NULL; });

$benim_macim_id = null;
if($kullanici_takim) {
    $benim_macim_id = $pdo->query("SELECT id FROM de_maclar WHERE hafta=$goster_hafta AND ev_skor IS NULL AND (ev=$kullanici_takim OR dep=$kullanici_takim)")->fetchColumn();
}

$sezon_tamam = false;
$sampiyon_takim = null;
try {
    $toplam_mac_de = $pdo->query("SELECT COUNT(*) FROM de_maclar WHERE sezon_yil=$sezon_yili")->fetchColumn();
    $kalan_de = $pdo->query("SELECT COUNT(*) FROM de_maclar WHERE sezon_yil=$sezon_yili AND ev_skor IS NULL")->fetchColumn();
    if($toplam_mac_de > 0 && $kalan_de == 0 && $hafta >= $max_hafta) {
        $sezon_tamam = true;
...
<head>
    ...
    <style>
        ...
        :root {
            ...
            --de-accent: #ff6b35;    /* Turuncu Vurgu */
            --de-dark: #0d0d0d;
        ...
        .btn-action-primary { background: var(--de-secondary); color: var(--de-dark); font-weight: 800; padding: 8px 20px; border-radius: 4px; text-decoration: none; border: none; transition: 0.3s;}
        .btn-action-primary:hover { background: #ef4444; color: #000; box-shadow: 0 0 15px var(--de-secondary); }
        ...
        .data-table th { padding: 1rem; color: var(--de-secondary); font-weight: 700; text-transform: uppercase; font-size: 0.75rem; border-bottom: 1px solid var(--border-color); text-align: center; }
        ...
        /* Bundesliga Bölgeleme Kuralları */
        ...
        .scorebug-container:hover { border-color: var(--de-secondary); box-shadow: 0 5px 15px rgba(220,38,38,0.15); transform: translateY(-2px);}
        ...
        .match-score { font-family: 'Oswald', sans-serif; font-size: 1.8rem; font-weight: 900; color: var(--de-secondary); line-height: 1; letter-spacing: 1px; text-shadow: 0 2px 4px rgba(0,0,0,0.8); }
        ...
        .action-btn { flex: 1; padding: 10px; text-align: center; text-decoration: none; color: var(--de-secondary); font-size: 0.85rem; font-weight: 700; text-transform: uppercase; transition: 0.2s; display: flex; justify-content: center; align-items: center; gap: 8px;}
        .action-btn:hover { background: var(--de-secondary); color: var(--de-dark) !important; }
        ...
        .event-time { font-family: 'Oswald'; font-weight: 700; color: var(--de-primary); flex-shrink: 0; min-width: 25px;}
        ...
        @keyframes pulse_glow {
            ...
            50% { box-shadow: 0 0 30px rgba(220,38,38,0.8), 0 0 50px rgba(200,16,46,0.3); }
...
<body>
    ...
    <nav class="pro-navbar">
        ...
        <div class="nav-menu d-none d-lg-flex gap-3">
            ...
            <a href="bundesliga.php" class="nav-link-item text-white fw-bold" style="text-shadow: 0 0 10px var(--de-secondary);"><i class="fa-solid fa-tv"></i> Maç Merkezi</a>
            ...
            <a href="bl_puan.php" class="nav-link-item"><i class="fa-solid fa-chart-pie"></i> İstatistik</a>
            <a href="bl_basin.php" class="nav-link-item"><i class="fa-solid fa-microphone"></i> Medya</a>
        ...
        <div class="d-flex gap-3">
            <?php if($kullanici_takim): ?>
                ...
                <?php if(!$sezon_tamam): ?>
                <a href="?action=sezonu_simule" class="btn-action-outline" style="background:#7f1d1d;border-color:#dc2626;color:#fca5a5;" onclick="return confirm('Bundesliga sezonu simüle edilecek. Devam?')">
    ...
    <div class="container-fluid py-3 px-4">
        <div style="background:linear-gradient(135deg,#1a0000,#7f1d1d);border:2px solid #d4af37;border-radius:12px;padding:20px 24px;margin-bottom:16px;text-align:center;">
            <div style="font-family:monospace;color:#d4af37;font-size:0.82rem;">=============================================</div>
            <div style="font-family:'Oswald',sans-serif;font-size:1.3rem;font-weight:900;color:#fff;margin:8px 0;">🏆 BUNDESLİGA <?=$sezon_yili?>/<?=$sezon_yili+1?> SEZONU ŞAMPİYONU 🏆</div>
            ...
            <div style="background:rgba(0,0,0,0.4);border:1px solid rgba(212,175,55,0.3);border-radius:6px;padding:10px 14px;margin-top:12px;font-family:monospace;font-size:0.78rem;color:#a3e635;text-align:left;max-width:400px;margin-left:auto;margin-right:auto;">
                ...
                $son_sampiyon['bundesliga'] = "<?=htmlspecialchars($sampiyon_takim['takim_adi'])?>";
            </div>
        </div>
    </div>
    <?php endif; ?>
    ...
    <?php if(!$kullanici_takim): ?>
        <div class="container py-5 text-center" style="max-width: 1200px;">
            ...
            <h1 class="font-oswald mb-4" style="font-size: 4rem; color: var(--de-secondary); text-shadow: 0 0 20px rgba(220,38,38,0.3);">BUNDESLIGA'YA HOŞ GELDİN</h1>
            <p class="text-white mb-5 fs-5">Almanya'nın en prestijli liginde yönetmek istediğin kulübü seç.</p>
            
            <div class="row g-4 justify-content-center">
                <?php 
                $secilebilir = $pdo->query("SELECT * FROM de_takimlar ORDER BY takim_adi ASC")->fetchAll(PDO::FETCH_ASSOC);
                ...
                <div class="col-md-3 col-sm-4 col-6">
                    <div class="panel-card p-4 text-center hover-lift" onclick="window.location='?action=takim_sec&tid=<?= $t['id'] ?>';">
                        ...
                        <small style="color: var(--de-secondary);">Hücum: <?= $t['hucum'] ?> | Savunma: <?= $t['savunma'] ?></small>
                    </div>
                </div>
                <?php endforeach; ?>
            </div>
                                    ...
                                    <tr class="<?= $row_class ?>">
                                        ...
                                        <td>
                                            <div class="cell-club">
                                                ...
                                                <span style="<?= $t['id'] == $kullanici_takim ? 'color: var(--de-secondary);' : '' ?>"><?= htmlspecialchars($t['takim_adi']) ?></span>
                                            </div>
                                        </td>
                                        ...
                                        <td class="font-oswald fs-5" style="color: <?= $t['id'] == $kullanici_takim ? 'var(--de-secondary)' : '#fff' ?>;"><?= $t['puan'] ?></td>
                                    </tr>
                                    <?php endforeach; ?>
                                </tbody>
                            </table>
                ...
                <div class="col-xl-5">
                    <div class="panel-card" style="height: 100%; max-height:850px;">
                        <div class="panel-header">
                            ...
                            <span class="font-oswald text-white fs-5">SPIELTAG <?= $goster_hafta ?></span>
                        ...
                        <div class="fixture-wrapper">
                            ...
                            ?>
                                <div class="scorebug-container" style="border-color: rgba(220,38,38,0.5); box-shadow: 0 0 10px rgba(220,38,38,0.1);">
                                    <div class="score-grid">
                                        <div class="team-block home">
                                            <span class="team-name"><?= htmlspecialchars($mac['ev_ad']) ?></span>
                                            <img src="<?= htmlspecialchars($mac['ev_logo']) ?>" class="team-logo">
                                    ...
                                    <div class="events-grid">
                                        <div class="event-col home">
                                            ...
                                            foreach($ev_olaylar as $o) {
                                                ...
                                                echo "<div class='event-item justify-content-end'>$asist <span class='event-player'>$oyuncu</span> <span class='event-time'>$dakika'</span> <i class='fa-solid fa-futbol' style='color:var(--de-secondary);'></i></div>"; 
                                                $yazilan_gol_ev++;
                                            }
                                        ...
                                        <div class="event-col away">
                                            ...
                                            foreach($dep_olaylar as $o) {
                                                ...
                                                $dakika = $o['dakika'] ?? rand(1,90);
                                                $asist = (isset($o['asist']) && $o['asist'] !== '-') ? "<span class='event-assist'>(A: " . htmlspecialchars($o['asist']) . ")</span>" : "";
                                                echo "<div class='event-item'><i class='fa-solid fa-futbol' style='color:var(--de-secondary);'></i> <span class='event-time'>$dakika'</span> <span class='event-player'>$oyuncu</span> $asist</div>"; 
                                                $yazilan_gol_dep++;
                                            }
    ...
    <?php if($kullanici_takim): ?>
    <div class="d-flex d-lg-none fixed-bottom p-2 justify-content-around align-items-center border-top" style="background: rgba(21,0,3,0.97); backdrop-filter: blur(10px); z-index:2000; padding-bottom: 15px !important; border-top-color: var(--de-secondary) !important;">
        <a href="bundesliga.php" class="text-decoration-none text-center fw-bold" style="font-size: 0.8rem; width: 20%; color: var(--de-secondary);">
            <i class="fa-solid fa-tv fs-5 mb-1 d-block"></i> Fikstür
        </a>
        <a href="bl_kadro.php" class="text-secondary text-decoration-none text-center" style="font-size: 0.8rem; width: 20%;">
            <i class="fa-solid fa-users fs-5 mb-1 d-block text-white"></i> Kadro
        </a>
        <a href="bl_transfer.php" class="text-secondary text-decoration-none text-center" style="font-size: 0.8rem; width: 20%;">
            <i class="fa-solid fa-comments-dollar fs-5 mb-1 d-block text-white"></i> Transfer
        </a>
        <a href="bl_puan.php" class="text-secondary text-decoration-none text-center" style="font-size: 0.8rem; width: 20%;">
            <i class="fa-solid fa-chart-pie fs-5 mb-1 d-block text-white"></i> Veri
        </a>
        <a href="bl_basin.php" class="text-secondary text-decoration-none text-center" style="font-size: 0.8rem; width: 20%;">
    ...
    <div style="position:fixed; bottom: 80px; left:50%; transform:translateX(-50%); z-index: 3000;">
        <a href="bl_sezon_gecisi.php" class="btn fw-bold py-3 px-5" style="background:var(--de-secondary); color:var(--de-dark); border-radius:50px; box-shadow: 0 5px 20px rgba(220,38,38,0.5); animation: pulse_glow 2s infinite; font-size:1.1rem;">
