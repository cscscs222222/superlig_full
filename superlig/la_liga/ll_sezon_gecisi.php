...
if(isset($_POST['yeni_sezona_gec'])) {
    ...
    foreach ([$puan_durumu[4] ?? null, $puan_durumu[5] ?? null] as $uel_takim) {
        ...
        try {
            ...
            if (!$var_mi) {
                $stmt = $pdo->prepare("INSERT INTO uel_takimlar (takim_adi, logo, hucum, savunma, butce, lig) VALUES (?, ?, ?, ?, ?, 'La Liga')");
                $stmt->execute([$uel_takim['takim_adi'], $uel_takim['logo'], $uel_takim['hucum'], $uel_takim['savunma'], 18000000]);
                $yeni_uel_id = $pdo->lastInsertId();
                $oyuncular_uel = $pdo->query("SELECT * FROM es_oyuncular WHERE takim_id = {$uel_takim['id']}")->fetchAll(PDO::FETCH_ASSOC);
                foreach ($oyuncular_uel as $o) {
                    $stmt_o = $pdo->prepare("INSERT INTO uel_oyuncular (takim_id, isim, mevki, ovr, yas, fiyat, lig, ilk_11, yedek) VALUES (?, ?, ?, ?, ?, ?, 'La Liga', ?, ?)");
    ...
    foreach ([$puan_durumu[6] ?? null, $puan_durumu[7] ?? null] as $uecl_takim) {
        ...
        try {
            ...
            if (!$var_mi) {
                $stmt = $pdo->prepare("INSERT INTO uecl_takimlar (takim_adi, logo, hucum, savunma, butce, lig) VALUES (?, ?, ?, ?, ?, 'La Liga')");
                ...
                $oyuncular_uecl = $pdo->query("SELECT * FROM es_oyuncular WHERE takim_id = {$uecl_takim['id']}")->fetchAll(PDO::FETCH_ASSOC);
                foreach ($oyuncular_uecl as $o) {
                    $stmt_o = $pdo->prepare("INSERT INTO uecl_oyuncular (takim_id, isim, mevki, ovr, yas, fiyat, lig, ilk_11, yedek) VALUES (?, ?, ?, ?, ?, ?, 'La Liga', ?, ?)");
