...
if(isset($_GET['action'])) {
    ...
    if($action == 'bu_sezon_sifirla') {
        ...
        $pdo->exec("UPDATE es_oyuncular SET sezon_gol=0, sezon_asist=0, form=6, fitness=100, moral=80, ceza_hafta=0, sakatlik_hafta=0, mac_puani_ort=6.00");
        $pdo->exec("UPDATE es_ayar SET hafta=1");
        header("Location: la_liga.php?sifirla=bu_sezon"); exit;
    }
    ...
    if($action == 'tum_sezon_sifirla') {
        $pdo->exec("TRUNCATE TABLE es_maclar");
        $pdo->exec("UPDATE es_takimlar SET puan=0, galibiyet=0, beraberlik=0, malubiyet=0, atilan_gol=0, yenilen_gol=0");
        $pdo->exec("UPDATE es_oyuncular SET sezon_gol=0, sezon_asist=0, toplam_mac=0, toplam_gol=0, form=6, fitness=100, moral=80, ceza_hafta=0, sakatlik_hafta=0, mac_puani_ort=6.00");
        try { $pdo->exec("UPDATE es_ayar SET hafta=1, sezon_yil=2025, gecen_sezon_sampiyon=NULL"); } catch(Throwable $e) { $pdo->exec("UPDATE es_ayar SET hafta=1, sezon_yil=2025"); }
        try { $pdo->exec("TRUNCATE TABLE es_haberler"); } catch(Throwable $e) {}
        header("Location: la_liga.php?sifirla=tum_sezon"); exit;
    }
    ...
    if($action == 'sezonu_simule') {
        for($h = $hafta; $h <= $max_hafta; $h++) {
            $maclar_h = $pdo->query("SELECT m.id, m.ev, m.dep, t1.hucum as ev_hucum, t1.savunma as ev_savunma, t2.hucum as dep_hucum, t2.savunma as dep_savunma FROM es_maclar m JOIN es_takimlar t1 ON m.ev=t1.id JOIN es_takimlar t2 ON m.dep=t2.id WHERE m.hafta=$h AND m.ev_skor IS NULL")->fetchAll(PDO::FETCH_ASSOC);
            foreach($maclar_h as $m) {
                $skorlar = $engine->gercekci_skor_hesapla($m['ev'], $m['dep'], $m);
                $ev_skor = $skorlar['ev']; $dep_skor = $skorlar['dep'];
                $ev_detay = $engine->mac_olay_uret($m['ev'], $ev_skor);
                $dep_detay = $engine->mac_olay_uret($m['dep'], $dep_skor);
                $pdo->prepare("UPDATE es_maclar SET ev_skor=?,dep_skor=?,ev_olaylar=?,dep_olaylar=?,ev_kartlar=?,dep_kartlar=? WHERE id=?")->execute([$ev_skor,$dep_skor,$ev_detay['olaylar'],$dep_detay['olaylar'],$ev_detay['kartlar'],$dep_detay['kartlar'],$m['id']]);
                $pdo->exec("UPDATE es_takimlar SET atilan_gol=atilan_gol+$ev_skor,yenilen_gol=yenilen_gol+$dep_skor WHERE id={$m['ev']}");
                $pdo->exec("UPDATE es_takimlar SET atilan_gol=atilan_gol+$dep_skor,yenilen_gol=yenilen_gol+$ev_skor WHERE id={$m['dep']}");
                if($ev_skor>$dep_skor){$pdo->exec("UPDATE es_takimlar SET puan=puan+3,galibiyet=galibiyet+1 WHERE id={$m['ev']}");$pdo->exec("UPDATE es_takimlar SET malubiyet=malubiyet+1 WHERE id={$m['dep']}");}
                elseif($ev_skor==$dep_skor){$pdo->exec("UPDATE es_takimlar SET puan=puan+1,beraberlik=beraberlik+1 WHERE id IN ({$m['ev']},{$m['dep']})");}
        ...
        $pdo->exec("UPDATE es_ayar SET hafta=$max_hafta");
        header("Location: la_liga.php?sezon_bitti=1"); exit;
    }
}
...
$sampiyon_takim = null;
try {
    $toplam_mac_es = $pdo->query("SELECT COUNT(*) FROM es_maclar WHERE sezon_yil=$sezon_yili")->fetchColumn();
    $kalan_es = $pdo->query("SELECT COUNT(*) FROM es_maclar WHERE sezon_yil=$sezon_yili AND ev_skor IS NULL")->fetchColumn();
    if($toplam_mac_es > 0 && $kalan_es == 0 && $hafta >= $max_hafta) {
        $sezon_tamam = true;
...
<body>
    ...
    <div class="container-fluid px-4 pt-3">
        ...
        <div class="reset-alert warning"><i class="fa-solid fa-rotate-left me-2"></i>🔄 Tüm sezonlar sıfırlandı. La Liga geçmişi fabrika ayarlarına döndürüldü.</div>
        <?php endif; ?>
    </div>
    <?php endif; ?>
    ...
    <div class="container-fluid py-3 px-4">
        <div style="background:linear-gradient(135deg,#1a0008,#7f1d1d);border:2px solid #f5c518;border-radius:12px;padding:20px 24px;margin-bottom:16px;text-align:center;">
            <div style="font-family:monospace;color:#f5c518;font-size:0.82rem;">=============================================</div>
            <div style="font-family:'Oswald',sans-serif;font-size:1.3rem;font-weight:900;color:#fff;margin:8px 0;">🏆 LA LİGA <?=$sezon_yili?>/<?=$sezon_yili+1?> SEZONU ŞAMPİYONU 🏆</div>
            <div style="font-family:'Oswald',sans-serif;font-size:1.6rem;font-weight:900;color:#f5c518;margin:6px 0;">★ <?=htmlspecialchars($sampiyon_takim['takim_adi'])?> ★</div>
            <div style="font-family:monospace;color:#f5c518;font-size:0.82rem;">=============================================</div>
            ...
            <div style="margin-top:12px;font-size:0.8rem;max-width:400px;margin-left:auto;margin-right:auto;text-align:left;">
                <div style="color:#f5c518;font-weight:700;margin-bottom:6px;text-transform:uppercase;font-size:0.72rem;">Final Tablosu — İlk 5</div>
                ...
                <div style="display:flex;gap:8px;padding:3px 0;color:#fff;border-bottom:1px solid rgba(255,255,255,0.1);">
                    <span style="width:20px;color:#f5c518;font-weight:700;"><?=$idx+1?></span>
                    ...
                    <span style="color:#f5c518;font-weight:900;"><?=$st['puan']?>P</span>
                </div>
                <?php endforeach; ?>
            </div>
            <div style="background:rgba(0,0,0,0.4);border:1px solid rgba(245,197,24,0.3);border-radius:6px;padding:10px 14px;margin-top:12px;font-family:monospace;font-size:0.78rem;color:#a3e635;text-align:left;max-width:400px;margin-left:auto;margin-right:auto;">
                <div style="color:#94a3b8;margin-bottom:4px;">// index.php için son şampiyon güncellemesi</div>
                $son_sampiyon['la_liga'] = "<?=htmlspecialchars($sampiyon_takim['takim_adi'])?>";
            </div>
        </div>
    </div>
    <?php endif; ?>
