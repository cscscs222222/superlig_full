<?php
// ==============================================================================
// LA LIGA - KADRO VE TAKTİK MERKEZİ (RED & GOLD SPANISH THEME)
// ==============================================================================
include '../db.php';
...
if (isset($_POST['taktik_kaydet'])) {
    ...
    $stmt = $pdo->prepare("UPDATE es_takimlar SET dizilis=?, oyun_tarzi=?, pres=?, tempo=? WHERE id=?");
    $stmt->execute([$dizilis, $oyun_tarzi, $pres, $tempo, $kullanici_takim_id]);
    
    $mesaj = "İspanyol taktikleri başarıyla güncellendi! ¡Vamos!";
    $mesaj_tipi = "success";
...
if (isset($_GET['islem']) && isset($_GET['oyuncu_id'])) {
    ...
    $benim_mi = $pdo->query("SELECT COUNT(*) FROM es_oyuncular WHERE id=$oyuncu_id AND takim_id=$kullanici_takim_id")->fetchColumn();
    
    if ($benim_mi) {
        if ($islem == 'ilk11_yap') {
            $ilk11_sayisi = $pdo->query("SELECT COUNT(*) FROM es_oyuncular WHERE takim_id=$kullanici_takim_id AND ilk_11=1")->fetchColumn();
            if ($ilk11_sayisi >= 11) {
                $mesaj = "Sahaya en fazla 11 oyuncu sürebilirsiniz! Önce birini yedeğe çekin.";
                $mesaj_tipi = "danger";
            } else {
                $pdo->exec("UPDATE es_oyuncular SET ilk_11=1, yedek=0 WHERE id=$oyuncu_id");
            }
        } elseif ($islem == 'yedek_yap') {
            $pdo->exec("UPDATE es_oyuncular SET ilk_11=0, yedek=1 WHERE id=$oyuncu_id");
        } elseif ($islem == 'kadro_disi_yap') {
            $pdo->exec("UPDATE es_oyuncular SET ilk_11=0, yedek=0 WHERE id=$oyuncu_id");
        }
        
        if(!$mesaj) {
            header("Location: ll_kadro.php"); 
            exit;
        }
    }
}
...
$tum_kadro = $pdo->query("SELECT * FROM es_oyuncular WHERE takim_id = $kullanici_takim_id ORDER BY CASE mevki WHEN 'K' THEN 1 WHEN 'D' THEN 2 WHEN 'OS' THEN 3 WHEN 'F' THEN 4 END, ovr DESC")->fetchAll(PDO::FETCH_ASSOC);
...
$takim_kalitesi = $pdo->query("SELECT AVG(ovr) FROM es_oyuncular WHERE takim_id = $kullanici_takim_id AND ilk_11 = 1")->fetchColumn();
...
<head>
    ...
    <title>Kadro & Taktik | La Liga</title>
    ...
    <style>
        ...
        .btn-taktik { background: var(--es-secondary); color: var(--es-dark); font-weight: 800; border: none; padding: 10px; width: 100%; border-radius: 6px; transition: 0.3s;}
        .btn-taktik:hover { background: #ffd700; color: #000; box-shadow: 0 0 15px var(--es-secondary); transform: translateY(-2px);}
        ...
        .action-icon.up:hover { color: var(--es-secondary); transform: translateY(-2px); text-shadow: 0 0 5px var(--es-secondary);}
        ...
        .status-banned { background: rgba(245,197,24,0.2); color: var(--color-warning); border: 1px solid var(--color-warning);}
        
        .form-select { background-color: rgba(0,0,0,0.3); border: 1px solid var(--border-color); color: #fff; font-weight: 600;}
        .form-select:focus { border-color: var(--es-secondary); box-shadow: 0 0 0 0.25rem rgba(245,197,24,0.25); background-color: rgba(0,0,0,0.5); color: #fff;}
    </style>
</head>
<body>
    ...
    <nav class="pro-navbar">
        ...
        <div class="nav-menu d-none d-lg-flex gap-3">
            ...
            <a href="ll_kadro.php" class="nav-link-item text-white fw-bold" style="text-shadow: 0 0 10px var(--es-secondary);"><i class="fa-solid fa-users"></i> Kadro / Taktik</a>
        </div>

        <div class="d-flex gap-3">
            <a href="la_liga.php" class="nav-link-item text-danger border-danger" style="border: 1px solid; padding: 8px 20px; border-radius: 4px;">
    ...
    <div class="hero-banner">
        ...
        <h1 class="font-oswald m-0 text-white" style="font-size: 3rem; text-shadow: 0 0 20px rgba(245,197,24,0.3);"><?= htmlspecialchars($takim['takim_adi']) ?></h1>
        <p class="fw-bold mt-2" style="color: var(--es-secondary);">
        ...
        <div class="row g-4">
            <div class="col-xl-3 col-lg-4">
                <div class="panel-card p-4">
                    <h3 class="font-oswald mb-4 text-center border-bottom pb-3" style="border-color:var(--border-color); color:var(--es-primary);">
                        <i class="fa-solid fa-chess-board"></i> Taktik Odası
                    </h3>
                    
                    <form method="POST">
                        <div class="mb-4">
                            <label class="form-label fw-bold text-uppercase" style="font-size:0.8rem; color:var(--es-secondary);">Diziliş (Formasyon)</label>
                        ...
                        <div class="mb-4">
                            <label class="form-label fw-bold text-uppercase" style="font-size:0.8rem; color:var(--es-secondary);">Oyun Tarzı</label>
                        ...
                        <div class="mb-4">
                            <label class="form-label fw-bold text-uppercase" style="font-size:0.8rem; color:var(--es-secondary);">Takım Presi</label>
                        ...
                        <div class="mb-4">
                            <label class="form-label fw-bold text-uppercase" style="font-size:0.8rem; color:var(--es-secondary);">Oyun Temposu</label>
                            <select name="tempo" class="form-select">
                                ...
                                <option value="Çok Hızlı" <?= ($takim['tempo'] ?? '')==='Çok Hızlı'?'selected':'' ?>>İspanyol Temposu (Çok Hızlı)</option>
                            </select>
                        </div>
            ...
            <div class="col-xl-9 col-lg-8">
                ...
                <div class="panel-card mb-4" style="border-color: var(--es-secondary);">
                    <div class="panel-header text-white" style="background: rgba(245, 197, 24, 0.1); border-bottom-color: var(--es-secondary);">
                        <i class="fa-solid fa-shirt" style="color:var(--es-secondary);"></i> ONCE TITULAR (İlk 11)
                    </div>
                ...
                <div class="panel-card" style="border-color: rgba(200, 16, 46, 0.3);">
                    <div class="panel-header" style="background: rgba(200, 16, 46, 0.05); color: var(--es-primary); border-bottom-color: rgba(200, 16, 46, 0.2);">
    ...
    <div class="d-flex d-lg-none fixed-bottom p-2 justify-content-around align-items-center border-top" style="background: rgba(21,0,3,0.95); backdrop-filter: blur(10px); z-index:2000; padding-bottom: 15px !important; border-top-color: var(--es-secondary) !important;">
        ...
        <a href="ll_kadro.php" class="text-decoration-none text-center fw-bold" style="font-size: 0.8rem; width: 33%; color: var(--es-secondary);">
            <i class="fa-solid fa-users fs-5 mb-1 d-block"></i> Kadro
        </a>
        <a href="ll_transfer.php" class="text-secondary text-decoration-none text-center" style="font-size: 0.8rem; width: 33%;">
