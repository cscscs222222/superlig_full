<?php
// ==============================================================================
// LA LIGA - TAKIM PROFİLİ VE SCOUT EKRANI (RED & GOLD SPANISH THEME)
// ==============================================================================
include '../db.php';

$takim_id = isset($_GET['id']) ? (int)$_GET['id'] : 0;
if(!$takim_id) { header("Location: la_liga.php"); exit; }
...
$takim = $pdo->query("SELECT * FROM es_takimlar WHERE id = $takim_id")->fetch(PDO::FETCH_ASSOC);
if(!$takim) { header("Location: la_liga.php"); exit; }

// Oyuncuları Çek (Mevkiye ve Güce göre sıralı)
$oyuncular = $pdo->query("SELECT * FROM es_oyuncular WHERE takim_id = $takim_id ORDER BY CASE mevki WHEN 'K' THEN 1 WHEN 'D' THEN 2 WHEN 'OS' THEN 3 WHEN 'F' THEN 4 END, ovr DESC")->fetchAll(PDO::FETCH_ASSOC);

$takim_kalitesi = $pdo->query("SELECT AVG(ovr) FROM es_oyuncular WHERE takim_id = $takim_id AND ilk_11 = 1")->fetchColumn();
...
<head>
    ...
    <title><?= htmlspecialchars($takim['takim_adi']) ?> | La Liga Scout</title>
    ...
    <style>
        ...
        .scout-box { background: rgba(0,0,0,0.4); border: 1px solid rgba(245,197,24,0.2); padding: 15px; border-radius: 8px; text-align: center; height: 100%; transition:0.3s;}
        .scout-box:hover { border-color: var(--es-secondary); transform: translateY(-3px); box-shadow: 0 5px 15px rgba(245,197,24,0.15);}
        ...
        /* LA LİGA FUT KART TASARIMI */
        .fut-card {
            background: linear-gradient(135deg, rgba(42,0,8,0.9), rgba(21,0,3,0.9));
        ...
        .fut-card:hover { transform: translateY(-8px); border-color: var(--es-secondary); box-shadow: 0 10px 25px rgba(245,197,24,0.2); }
        
        .fut-card.ilk11 { border-top: 3px solid var(--es-secondary); }
        .fut-card.yedek { border-top: 3px solid #fbbf24; }
        
        .fut-ovr { font-family: 'Oswald'; font-size: 2.2rem; font-weight: 900; line-height: 1; color: var(--es-secondary); text-shadow: 0 0 10px rgba(245,197,24,0.3);}
        .fut-pos { font-family: 'Oswald'; font-size: 1rem; color: var(--es-primary); font-weight: 700; margin-bottom: 10px;}
        .fut-name { font-weight: 700; font-size: 1rem; color: #fff; text-align: center; border-bottom: 1px solid rgba(245,197,24,0.2); padding-bottom: 10px; margin-bottom: 10px; width: 100%; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;}
...
<body>
    ...
    <div class="hero-banner">
        <img src="<?= htmlspecialchars($takim['logo']) ?>" alt="Logo" style="width: 110px; height: 110px; object-fit: contain; margin-bottom: 15px; filter: drop-shadow(0 0 25px rgba(245,197,24,0.4));">
        <h1 class="font-oswald m-0 text-white" style="font-size: 3.5rem; text-shadow: 0 0 20px rgba(245,197,24,0.3);"><?= htmlspecialchars($takim['takim_adi']) ?></h1>
        
        <?php if($takim['id'] == $kullanici_takim_id): ?>
            <span class="badge mt-2 fs-6" style="background:var(--es-secondary); color:var(--es-dark);"><i class="fa-solid fa-user-tie"></i> Senin Takımın</span>
        <?php else: ?>
            <span class="badge bg-dark border mt-2 fs-6" style="border-color:var(--es-primary) !important; color:var(--es-primary);"><i class="fa-solid fa-robot"></i> Rakip Kulüp</span>
        <?php endif; ?>
    </div>
    ...
    <div class="container py-5" style="max-width: 1400px;">
        ...
        <div class="panel-card mb-5">
            <div class="panel-header" style="color: var(--es-secondary);">
                <span class="font-oswald"><i class="fa-solid fa-magnifying-glass-chart me-2"></i> INFORME DE SCOUTING (Kulüp Scout Raporu)</span>
            </div>
            <div class="p-4">
                <div class="row g-3">
        ...
        <h3 class="font-oswald mb-4 border-bottom pb-2" style="border-color:var(--border-color); color:var(--es-primary);">
            ...
            <div class="col-xl-2 col-lg-3 col-md-4 col-6" style="transition: 0.3s;">
                <div class="fut-card <?= $card_class ?>">
                    ...
                    <div class="mt-3 w-100 text-center">
                        <span class="badge w-100" style="background:rgba(255,255,255,0.1); color:var(--es-secondary); font-size:0.7rem;"><?= $durum_text ?></span>
                    </div>
                    
                    <?php if($takim['id'] != $kullanici_takim_id): ?>
                        <a href="ll_transfer.php?q=<?= urlencode($o['isim']) ?>" class="btn btn-sm w-100 mt-2" style="background:var(--es-primary); color:#fff; font-weight:700; font-size:0.75rem; border:none;">TEKLİF YAP</a>
                    <?php endif; ?>
                </div>
            </div>
            <?php endforeach; ?>
    ...
    <div class="d-flex d-lg-none fixed-bottom p-2 justify-content-around align-items-center border-top" style="background: rgba(21,0,3,0.95); backdrop-filter: blur(10px); z-index:2000; padding-bottom: 15px !important; border-top-color: var(--es-secondary) !important;">
        ...
        <a href="ll_puan.php" class="text-secondary text-decoration-none text-center" style="font-size: 0.8rem; width: 25%;">
