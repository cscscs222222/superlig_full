<?php
// ==============================================================================
// LIGA NOS - BASIN ODASI VE PSİKOLOJİ MERKEZİ (GREEN & RED PORTUGUESE THEME)
// ==============================================================================
include '../db.php';

function sutunEkleBasinLn($pdo, $tablo, $sutun, $tip) {
    try {
        if($pdo->query("SHOW COLUMNS FROM `$tablo` LIKE '$sutun'")->rowCount() == 0)
            $pdo->exec("ALTER TABLE `$tablo` ADD `$sutun` $tip");
    } catch(Throwable $e) {}
}
sutunEkleBasinLn($pdo, 'pt_oyuncular', 'moral', 'INT DEFAULT 80');
sutunEkleBasinLn($pdo, 'pt_ayar', 'son_basin_haftasi', 'INT DEFAULT 0');
...
if(isset($_POST['moralleri_guncelle'])) {
    $pdo->exec("UPDATE pt_oyuncular SET moral = LEAST(100, moral + 5) WHERE takim_id = $kullanici_takim_id AND ilk_11 = 1");
    $pdo->exec("UPDATE pt_oyuncular SET moral = GREATEST(10, moral - 15) WHERE takim_id = $kullanici_takim_id AND yedek = 1 AND ovr >= 80");
    $pdo->exec("UPDATE pt_oyuncular SET moral = GREATEST(10, moral - 5) WHERE takim_id = $kullanici_takim_id AND yedek = 1 AND ovr < 80");
    $mesaj = "Portekiz basını yedek kalan yıldızları konuşuyor! Moraller güncellendi.";
    $mesaj_tipi = "warning";
}

// TAKIM YEMEĞİ
if(isset($_POST['takim_yemegi'])) {
    $yemek_maliyeti = 200000;
    if($takim['butce'] >= $yemek_maliyeti) {
        $pdo->exec("UPDATE pt_takimlar SET butce = butce - $yemek_maliyeti WHERE id = $kullanici_takim_id");
        $pdo->exec("UPDATE pt_oyuncular SET moral = LEAST(100, moral + 20) WHERE takim_id = $kullanici_takim_id");
        $mesaj = "Lizbon'da takım yemeği! Takım ruhu tavan yaptı! (+20 Moral, -€200K)";
        $mesaj_tipi = "success";
        $takim['butce'] -= $yemek_maliyeti;
    } else {
        $mesaj = "Bütçeniz yetersiz! Takım yemeği için €200K gerekiyor."; $mesaj_tipi = "danger";
    }
}
...
if(isset($_POST['cevap_ver'])) {
    ...
    if(($ayar['son_basin_haftasi'] ?? 0) == $guncel_hafta) {
        $mesaj = "Bu hafta zaten A Bola'ya konuştunuz! Haftaya tekrar gelin."; $mesaj_tipi = "warning";
    } else {
        if($cevap_id == 1) {
            $pdo->exec("UPDATE pt_oyuncular SET moral = GREATEST(10, moral - 10) WHERE takim_id = $kullanici_takim_id");
            $pdo->exec("UPDATE pt_takimlar SET butce = butce + 800000 WHERE id = $kullanici_takim_id");
            $mesaj = "Sert tavrınız yönetimi memnun etti (+€800K) ama oyuncular gerildi (-10 Moral).";
        } elseif($cevap_id == 2) {
            $pdo->exec("UPDATE pt_oyuncular SET moral = LEAST(100, moral + 15) WHERE takim_id = $kullanici_takim_id");
            $mesaj = "Takımı basının önüne atmadınız! Soyunma odasında harika bir atmosfer var (+15 Moral)!";
        } elseif($cevap_id == 3) {
            $pdo->exec("UPDATE pt_takimlar SET butce = GREATEST(0, butce - 400000) WHERE id = $kullanici_takim_id");
            $pdo->exec("UPDATE pt_oyuncular SET moral = LEAST(100, moral + 25) WHERE takim_id = $kullanici_takim_id");
            $mesaj = "Agresif açıklamalarınız Portekiz'i salladı (+25 Moral) ama FPF (Portekiz Federasyonu) ceza kesti (-€400K)!";
        }
        $pdo->exec("UPDATE pt_ayar SET son_basin_haftasi = $guncel_hafta WHERE id = 1");
        $mesaj_tipi = "success";
        $takim = $pdo->query("SELECT * FROM pt_takimlar WHERE id = $kullanici_takim_id")->fetch(PDO::FETCH_ASSOC);
    }
}

$oyuncular = $pdo->query("SELECT * FROM pt_oyuncular WHERE takim_id = $kullanici_takim_id ORDER BY moral ASC")->fetchAll(PDO::FETCH_ASSOC);
$ortalama_moral = $pdo->query("SELECT AVG(moral) FROM pt_oyuncular WHERE takim_id = $kullanici_takim_id")->fetchColumn();
$ortalama_moral = $ortalama_moral ? round($ortalama_moral) : 80;

$basin_sorulari = [
    "Benfica, Porto ve Sporting üçgeninde kendinizi nasıl konumlandırıyorsunuz?",
    "Taça de Portugal bu sezon stratejik önceliğiniz mi?",
    "Portekizli genç oyunculara ne kadar şans vereceksiniz?",
    "Şampiyonlar Ligi play-off turunu geçme hedefi var mı?",
    "Taraftar desteği performansı nasıl etkiliyor?",
];
...
<head>
    ...
    <title>Basın Odası | Liga NOS</title>
    ...
    <style>
        ...
        .player-moral-row:hover { background:rgba(0,102,0,0.06); }
        .press-question { background:rgba(0,102,0,0.15); border:1px solid rgba(0,102,0,0.4); border-radius:12px; padding:20px; margin-bottom:16px; }
        ...
        .answer-btn:hover { background:rgba(0,102,0,0.2); border-color:var(--pt-primary); }
        ...
        .btn-action { background:linear-gradient(135deg,var(--pt-primary),#009900); color:#fff; border:none; border-radius:8px; padding:12px 20px; font-weight:700; cursor:pointer; width:100%; transition:0.2s; }
...
<nav class="pro-navbar">
    ...
    <div class="d-none d-lg-flex gap-2">
        ...
        <a href="ln_basin.php" class="nav-link-item" style="color:#fff;"><i class="fa-solid fa-microphone"></i> Basın</a>
    </div>
            ...
            <div class="panel-card">
                ...
                <div class="p-4 text-center">
                    ...
                    <form method="POST" class="action-form">
                        ...
                        <button type="submit" name="takim_yemegi" class="btn-action" style="background:linear-gradient(135deg,#7e22ce,#9333ea);"
                            onclick="return confirm('Takım yemeği için €200K ödenecek. Onaylıyor musunuz?')">
                            <i class="fa-solid fa-utensils me-2"></i>Lizbon'da Takım Yemeği (€200K)
                        </button>
                    </form>
                </div>
                <div style="max-height:350px;overflow-y:auto;">
                    ...
                    <div class="player-moral-row">
                        <div style="width:36px;height:36px;background:var(--pt-primary);border-radius:8px;display:flex;align-items:center;justify-content:center;font-family:'Oswald';font-weight:900;color:#fff;font-size:1rem;flex-shrink:0;"><?=$o['ovr']?></div>
        ...
        <div class="col-lg-7">
            <div class="panel-card">
                <div class="panel-header">
                    <h5><i class="fa-solid fa-newspaper me-2"></i>A Bola Basın Toplantısı — Hafta <?=$guncel_hafta?></h5>
                </div>
                <div class="p-4">
                    ...
                    <div style="background:rgba(255,193,7,0.1);border:1px solid rgba(255,193,7,0.3);border-radius:10px;padding:16px;text-align:center;color:#fbbf24;">
                        ...
                        <div style="font-weight:700;">Bu hafta A Bola ile konuştunuz!</div>
                        <div style="font-size:0.85rem;margin-top:4px;color:var(--muted);">Haftaya yeni sorular için gelin.</div>
                    </div>
                    <?php else: ?>
                    <div class="press-question">
                        <div style="font-size:0.75rem;color:var(--pt-secondary);text-transform:uppercase;letter-spacing:1px;margin-bottom:8px;font-weight:700;">📰 A Bola Sorusu</div>
                        ...
                        <form method="POST">
                            ...
                            <button type="submit" name="cevap_ver" class="answer-btn">
                                ...
                                <div>
                                    ...
                                    <div style="font-size:0.8rem;color:var(--muted);">+€800K Yönetim Desteği, -10 Moral</div>
                                </div>
                            </button>
                        </form>
                        <form method="POST">
                        ...
                        <form method="POST">
                            ...
                            <button type="submit" name="cevap_ver" class="answer-btn">
                                ...
                                <div>
                                    ...
                                    <div style="font-size:0.8rem;color:var(--muted);">+25 Moral, -€400K FPF Cezası</div>
                                </div>
                            </button>
                        </form>
                    </div>
                    ...
                    <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.07);border-radius:10px;padding:16px;margin-top:16px;">
                        ...
                        <div style="font-size:0.85rem;color:#94a3b8;line-height:1.8;">
                            ...
                            • <span style="color:var(--pt-gold);font-weight:700;">İlk 11 oyuncular</span> daha yüksek moral alır<br>
                            • <span style="color:var(--pt-secondary);font-weight:700;">Yedekte kalan yıldızlar</span> moral kaybeder
                        </div>
                    </div>
                </div>
            </div>
