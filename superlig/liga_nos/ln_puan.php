<?php
// ==============================================================================
// LIGA NOS - PUAN TABLOSU VE İSTATİSTİKLER
// ==============================================================================
include '../db.php';
...
$goller=[]; $asistler=[];
try {
    $tum_maclar=$pdo->query("SELECT ev_olaylar,dep_olaylar FROM pt_maclar WHERE ev_skor IS NOT NULL")->fetchAll(PDO::FETCH_ASSOC);
...
<title>Liga NOS Puan Tablosu | Ultimate Manager</title>
...
<style>
:root { --pt-primary:#006600; --pt-secondary:#cc0000; --pt-gold:#d4af37; --bg:#0d0d0d; --panel:#1a1a1a; --border:rgba(0,102,0,0.25); }
...
.data-table th { padding:0.8rem 1rem; color:var(--pt-gold); font-weight:700; text-transform:uppercase; font-size:0.72rem; border-bottom:1px solid var(--border); text-align:center; }
...
.stat-rank { width:24px; text-align:center; font-family:'Oswald',sans-serif; font-weight:700; color:#94a3b8; }
.stat-name { flex:1; font-weight:600; color:#fff; }
.stat-val { font-family:'Oswald',sans-serif; font-weight:900; color:var(--pt-gold); font-size:1.1rem; min-width:32px; text-align:right; }
</style>
</head>
<body>
<nav class="pro-navbar">
    ...
    <div class="d-none d-lg-flex gap-2">
        ...
        <a href="ln_puan.php" class="nav-link-item" style="color:#fff;"><i class="fa-solid fa-chart-bar"></i> Puan Tablosu</a>
    ...
    <div class="row g-4">
        <div class="col-lg-7">
            <div class="panel-card">
                <div class="panel-header"><h5><i class="fa-solid fa-table-list me-2"></i>Puan Durumu — Liga NOS</h5></div>
                ...
                <div style="padding:12px 16px;font-size:0.75rem;color:#94a3b8;border-top:1px solid var(--border);display:flex;gap:16px;">
                    <span style="border-left:3px solid var(--pt-primary);padding-left:6px;">1-4: Şampiyonlar Ligi</span>
                    <span style="border-left:3px solid var(--pt-secondary);padding-left:6px;">5: Avrupa Ligi</span>
        ...
        <div class="col-lg-5">
            <div class="panel-card mb-4">
                ...
                <?php else: $r=1; foreach($top_goller as $isim=>$adet): ?>
                    <div class="stat-row"><div class="stat-rank"><?=$r++?></div><div class="stat-name"><?=htmlspecialchars($isim)?></div><div class="stat-val"><?=$adet?> ⚽</div></div>
            ...
            <div class="panel-card">
                ...
                <?php else: $r=1; foreach($top_asistler as $isim=>$adet): ?>
                    <div class="stat-row"><div class="stat-rank"><?=$r++?></div><div class="stat-name"><?=htmlspecialchars($isim)?></div><div class="stat-val"><?=$adet?> 🅰️</div></div>
                <?php endforeach; endif; ?>
                </div>
            </div>
        </div>
