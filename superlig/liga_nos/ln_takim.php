<?php
// ==============================================================================
// LIGA NOS - TAKIM PROFİLİ VE SCOUT EKRANI (GREEN & RED PORTUGUESE THEME)
// ==============================================================================
include '../db.php';

$takim_id = isset($_GET['id']) ? (int)$_GET['id'] : 0;
if(!$takim_id) { header("Location: liga_nos.php"); exit; }

$ayar = $pdo->query("SELECT * FROM pt_ayar LIMIT 1")->fetch(PDO::FETCH_ASSOC);
$kullanici_takim_id = $ayar['kullanici_takim_id'] ?? null;

$takim = $pdo->query("SELECT * FROM pt_takimlar WHERE id = $takim_id")->fetch(PDO::FETCH_ASSOC);
if(!$takim) { header("Location: liga_nos.php"); exit; }

$oyuncular = $pdo->query("SELECT * FROM pt_oyuncular WHERE takim_id = $takim_id ORDER BY CASE mevki WHEN 'K' THEN 1 WHEN 'D' THEN 2 WHEN 'OS' THEN 3 WHEN 'F' THEN 4 END, ovr DESC")->fetchAll(PDO::FETCH_ASSOC);
$takim_kalitesi = $pdo->query("SELECT AVG(ovr) FROM pt_oyuncular WHERE takim_id = $takim_id AND ilk_11 = 1")->fetchColumn();
$takim_kalitesi = $takim_kalitesi ? round($takim_kalitesi, 1) : 0;

function paraFormatlaLn($sayi) {
...
<head>
    ...
    <title><?=htmlspecialchars($takim['takim_adi'])?> | Liga NOS Scout</title>
    ...
    <style>
        :root {
            --pt-primary: #006600; --pt-secondary: #cc0000; --pt-gold: #d4af37;
            --bg: #0d0d0d; --panel: #111827; --border: rgba(0,102,0,0.3);
            --text: #f9fafb; --muted: #94a3b8;
        }
        body { background: var(--bg); color: var(--text); font-family: 'Inter', sans-serif; min-height: 100vh;
            background-image: radial-gradient(circle at 0% 0%, rgba(0,102,0,0.1) 0%, transparent 50%), radial-gradient(circle at 100% 100%, rgba(204,0,0,0.08) 0%, transparent 50%); }
        ...
        .scout-box { background: rgba(0,0,0,0.4); border: 1px solid rgba(0,102,0,0.2); padding: 15px; border-radius: 8px; text-align: center; height: 100%; transition: 0.3s; }
        .scout-box:hover { border-color: var(--pt-secondary); transform: translateY(-3px); }
        ...
        .fut-card { background: linear-gradient(135deg, rgba(0,102,0,0.3), rgba(10,10,10,0.9)); border: 1px solid rgba(0,102,0,0.3); border-radius: 12px; padding: 15px; position: relative; transition: all 0.3s ease; overflow: hidden; display: flex; flex-direction: column; align-items: center; box-shadow: 0 5px 15px rgba(0,0,0,0.5); }
        .fut-card:hover { transform: translateY(-8px); border-color: var(--pt-secondary); box-shadow: 0 10px 25px rgba(204,0,0,0.2); }
        .fut-card.ilk11 { border-top: 3px solid var(--pt-primary); }
        .fut-card.yedek { border-top: 3px solid #fbbf24; }
        .fut-ovr { font-family: 'Oswald'; font-size: 2.2rem; font-weight: 900; line-height: 1; color: var(--pt-gold); }
        .fut-pos { font-family: 'Oswald'; font-size: 1rem; color: var(--pt-secondary); font-weight: 700; margin-bottom: 10px; }
        .fut-name { font-weight: 700; font-size: 1rem; color: #fff; text-align: center; border-bottom: 1px solid rgba(0,102,0,0.3); padding-bottom: 10px; margin-bottom: 10px; width: 100%; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        ...
        .btn-teklif { background: var(--pt-secondary); color: #fff; border: none; border-radius: 6px; padding: 6px 10px; font-size: 0.75rem; font-weight: 700; width: 100%; margin-top: 8px; text-decoration: none; display: block; text-align: center; transition: 0.2s; }
        .btn-teklif:hover { background: var(--pt-primary); color: #fff; }
    </style>
</head>
<body>
...
<div class="hero-banner">
    <img src="<?=$takim['logo']?>" alt="Logo" style="width:110px;height:110px;object-fit:contain;margin-bottom:15px;filter:drop-shadow(0 0 25px rgba(0,102,0,0.5));">
    ...
    <span class="badge mt-2 fs-6" style="background:var(--pt-secondary);color:#fff;"><i class="fa-solid fa-user-tie"></i> Senin Takımın</span>
    <?php else: ?>
    <span class="badge bg-dark border mt-2 fs-6" style="border-color:var(--pt-primary)!important;color:#4ade80;"><i class="fa-solid fa-robot"></i> Rakip Kulüp</span>
    <?php endif; ?>
</div>

<div class="container py-5" style="max-width:1400px;">
    <div class="panel-card mb-5">
        <div class="panel-header" style="color:var(--pt-gold);"><span class="font-oswald"><i class="fa-solid fa-magnifying-glass-chart me-2"></i>SCOUT RAPORU</span></div>
        <div class="p-4">
            <div class="row g-3">
                ...
                <div class="col-md-2 col-6"><div class="scout-box"><div class="scout-title">Bütçe</div><div class="scout-val" style="color:#4ade80;font-size:1.1rem;"><?=paraFormatlaLn($takim['butce']??0)?></div></div></div>
            </div>
        </div>
    </div>

    <h3 class="font-oswald mb-4 border-bottom pb-2" style="border-color:var(--border);color:var(--pt-secondary);">
        ...
        <div class="col-xl-2 col-lg-3 col-md-4 col-6">
            <div class="fut-card <?=$card_class?>">
                ...
                <div class="mt-3 w-100 text-center">
                    <span class="badge w-100" style="background:rgba(255,255,255,0.1);color:var(--pt-gold);font-size:0.7rem;"><?=$durum_text?></span>
                </div>
                <?php if($takim['id'] != $kullanici_takim_id): ?>
                <a href="ln_transfer.php?q=<?=urlencode($o['isim'])?>" class="btn-teklif">TEKLİF YAP</a>
                <?php endif; ?>
            </div>
        </div>
        <?php endforeach; ?>
