<?php
// ==============================================================================
// LIGA NOS - KADRO VE TAKTİK MERKEZİ (GREEN & RED PORTUGUESE THEME)
// ==============================================================================
include '../db.php';

try {
    $pdo->exec("CREATE TABLE IF NOT EXISTS pt_ayar ( id INT AUTO_INCREMENT PRIMARY KEY, hafta INT DEFAULT 1, sezon_yil INT DEFAULT 2025, kullanici_takim_id INT DEFAULT NULL )");
    $ayar_sayisi = $pdo->query("SELECT COUNT(*) FROM pt_ayar")->fetchColumn();
    if($ayar_sayisi == 0) { $pdo->exec("INSERT INTO pt_ayar (hafta, sezon_yil) VALUES (1, 2025)"); }
} catch (Throwable $e) {}
...
if (isset($_POST['taktik_kaydet'])) {
    ...
    $stmt = $pdo->prepare("UPDATE pt_takimlar SET dizilis=?, oyun_tarzi=?, pres=?, tempo=? WHERE id=?");
    $stmt->execute([$dizilis, $oyun_tarzi, $pres, $tempo, $kullanici_takim_id]);
    $mesaj = "Liga NOS taktikleri başarıyla güncellendi!";
    $mesaj_tipi = "success";
...
if (isset($_GET['islem']) && isset($_GET['oyuncu_id'])) {
    ...
    $benim_mi  = $pdo->query("SELECT COUNT(*) FROM pt_oyuncular WHERE id=$oyuncu_id AND takim_id=$kullanici_takim_id")->fetchColumn();
    if ($benim_mi) {
        if ($islem == 'ilk11_yap') {
            $ilk11_sayisi = $pdo->query("SELECT COUNT(*) FROM pt_oyuncular WHERE takim_id=$kullanici_takim_id AND ilk_11=1")->fetchColumn();
            if ($ilk11_sayisi >= 11) {
                $mesaj = "Sahaya en fazla 11 oyuncu sürebilirsiniz! Önce birini yedeğe çekin.";
                $mesaj_tipi = "danger";
            } else {
                $pdo->exec("UPDATE pt_oyuncular SET ilk_11=1, yedek=0 WHERE id=$oyuncu_id");
            }
        } elseif ($islem == 'yedek_yap') {
            $pdo->exec("UPDATE pt_oyuncular SET ilk_11=0, yedek=1 WHERE id=$oyuncu_id");
        } elseif ($islem == 'kadro_disi_yap') {
            $pdo->exec("UPDATE pt_oyuncular SET ilk_11=0, yedek=0 WHERE id=$oyuncu_id");
        }
        if(!$mesaj) { header("Location: ln_kadro.php"); exit; }
    }
}

$tum_kadro = $pdo->query("SELECT * FROM pt_oyuncular WHERE takim_id = $kullanici_takim_id ORDER BY CASE mevki WHEN 'K' THEN 1 WHEN 'D' THEN 2 WHEN 'OS' THEN 3 WHEN 'F' THEN 4 END, ovr DESC")->fetchAll(PDO::FETCH_ASSOC);
...
$takim_kalitesi = $pdo->query("SELECT AVG(ovr) FROM pt_oyuncular WHERE takim_id = $kullanici_takim_id AND ilk_11 = 1")->fetchColumn();
...
<head>
    ...
    <title>Kadro & Taktik | Liga NOS</title>
    ...
    <style>
        :root {
            ...
            --bg: #0d0d0d; --panel: #1a1a1a; --border: rgba(0,102,0,0.25);
        ...
        .player-row:hover { background: rgba(0,102,0,0.08); }
        ...
        .btn-ilk11 { background: rgba(0,102,0,0.3); color: #4ade80; border: 1px solid rgba(74,222,128,0.3); }
        .btn-ilk11:hover { background: var(--pt-primary); color: #fff; }
        ...
        .section-title { font-family: 'Oswald', sans-serif; font-size: 1.2rem; color: var(--pt-gold); text-transform: uppercase; border-bottom: 2px solid var(--border); padding-bottom: 8px; margin: 24px 0 12px; }
        ...
        .stat-card { background: rgba(0,102,0,0.1); border: 1px solid var(--border); border-radius: 10px; padding: 14px 16px; text-align: center; }
        .stat-card-val { font-family: 'Oswald', sans-serif; font-size: 2rem; font-weight: 900; color: var(--pt-gold); }
...
<nav class="pro-navbar">
    ...
    <div class="d-none d-lg-flex gap-2">
        ...
        <a href="ln_kadro.php" class="nav-link-item" style="color:#fff;"><i class="fa-solid fa-users"></i> Kadro</a>
