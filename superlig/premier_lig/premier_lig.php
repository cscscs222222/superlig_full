...
if(isset($_GET['action'])) {
    ...
    if($action == 'bu_sezon_sifirla') {
        ...
        $pdo->exec("UPDATE pl_oyuncular SET sezon_gol=0, sezon_asist=0, form=6, fitness=100, moral=80, ceza_hafta=0, sakatlik_hafta=0, mac_puani_ort=6.00");
        $pdo->exec("UPDATE pl_ayar SET hafta=1");
        header("Location: premier_lig.php?sifirla=bu_sezon"); exit;
    }
    ...
    if($action == 'tum_sezon_sifirla') {
        $pdo->exec("TRUNCATE TABLE pl_maclar");
        $pdo->exec("UPDATE pl_takimlar SET puan=0, galibiyet=0, beraberlik=0, malubiyet=0, atilan_gol=0, yenilen_gol=0");
        $pdo->exec("UPDATE pl_oyuncular SET sezon_gol=0, sezon_asist=0, toplam_mac=0, toplam_gol=0, form=6, fitness=100, moral=80, ceza_hafta=0, sakatlik_hafta=0, mac_puani_ort=6.00");
        try { $pdo->exec("UPDATE pl_ayar SET hafta=1, sezon_yil=2025, gecen_sezon_sampiyon=NULL"); } catch(Throwable $e) { $pdo->exec("UPDATE pl_ayar SET hafta=1, sezon_yil=2025"); }
        try { $pdo->exec("TRUNCATE TABLE pl_haberler"); } catch(Throwable $e) {}
        header("Location: premier_lig.php?sifirla=tum_sezon"); exit;
    }
    ...
    if($action == 'sezonu_simule') {
        for($h = $hafta; $h <= $max_hafta; $h++) {
            $maclar_h = $pdo->query("SELECT m.id, m.ev, m.dep, t1.hucum as ev_hucum, t1.savunma as ev_savunma, t2.hucum as dep_hucum, t2.savunma as dep_savunma FROM pl_maclar m JOIN pl_takimlar t1 ON m.ev=t1.id JOIN pl_takimlar t2 ON m.dep=t2.id WHERE m.hafta=$h AND m.ev_skor IS NULL")->fetchAll(PDO::FETCH_ASSOC);
            foreach($maclar_h as $m) {
                $skorlar = $engine->gercekci_skor_hesapla($m['ev'], $m['dep'], $m);
                $ev_skor = $skorlar['ev']; $dep_skor = $skorlar['dep'];
                $ev_detay = $engine->mac_olay_uret($m['ev'], $ev_skor);
                $dep_detay = $engine->mac_olay_uret($m['dep'], $dep_skor);
                $pdo->prepare("UPDATE pl_maclar SET ev_skor=?,dep_skor=?,ev_olaylar=?,dep_olaylar=?,ev_kartlar=?,dep_kartlar=? WHERE id=?")->execute([$ev_skor,$dep_skor,$ev_detay['olaylar'],$dep_detay['olaylar'],$ev_detay['kartlar'],$dep_detay['kartlar'],$m['id']]);
                $pdo->exec("UPDATE pl_takimlar SET atilan_gol=atilan_gol+$ev_skor,yenilen_gol=yenilen_gol+$dep_skor WHERE id={$m['ev']}");
                $pdo->exec("UPDATE pl_takimlar SET atilan_gol=atilan_gol+$dep_skor,yenilen_gol=yenilen_gol+$ev_skor WHERE id={$m['dep']}");
                if($ev_skor>$dep_skor){$pdo->exec("UPDATE pl_takimlar SET puan=puan+3,galibiyet=galibiyet+1 WHERE id={$m['ev']}");$pdo->exec("UPDATE pl_takimlar SET malubiyet=malubiyet+1 WHERE id={$m['dep']}");}
                elseif($ev_skor==$dep_skor){$pdo->exec("UPDATE pl_takimlar SET puan=puan+1,beraberlik=beraberlik+1 WHERE id IN ({$m['ev']},{$m['dep']})");}
        ...
        $pdo->exec("UPDATE pl_ayar SET hafta=$max_hafta");
        header("Location: premier_lig.php?sezon_bitti=1"); exit;
    }
}
...
$sampiyon_takim = null;
try {
    $toplam_mac_pl = $pdo->query("SELECT COUNT(*) FROM pl_maclar WHERE sezon_yil=$sezon_yili")->fetchColumn();
    $kalan_pl = $pdo->query("SELECT COUNT(*) FROM pl_maclar WHERE sezon_yil=$sezon_yili AND ev_skor IS NULL")->fetchColumn();
    if($toplam_mac_pl > 0 && $kalan_pl == 0 && $hafta >= $max_hafta) {
        $sezon_tamam = true;
...
<body>
    ...
    <nav class="pro-navbar">
        <a href="premier_lig.php" class="nav-brand">
            ...
            <span class="font-oswald">PREMIER LEAGUE</span>
        </a>
        ...
        <div class="d-flex gap-3">
            <?php if($kullanici_takim): ?>
                ...
                <?php if(!$sezon_tamam): ?>
                <a href="?action=sezonu_simule" class="btn-action-outline" style="background:#7f1d1d;border-color:#dc2626;color:#fca5a5;" onclick="return confirm('Premier Lig sezonu simüle edilecek. Devam?')">
                    <i class="fa-solid fa-forward-step"></i> Sezonu Simüle Et
                </a>
                <?php endif; ?>
                <a href="?action=bu_sezon_sifirla" class="btn-reset-sezon" onclick="return confirm('Bu sezonu sıfırla?\n\nPremier League mevcut fikstür, puan tablosu ve maç sonuçları silinecek.')"><i class="fa-solid fa-broom me-1"></i>Bu Sezonu Sıfırla</a>
                <a href="?action=tum_sezon_sifirla" class="btn-reset-tum" onclick="return confirm('⚠️ TÜM SEZONLARI SIFIRLA!\n\nPremier League TÜM GEÇMİŞİ silinecek!\n\nBu işlem geri alınamaz!')"><i class="fa-solid fa-fire me-1"></i>Tüm Sezonları Sıfırla</a>
            <?php endif; ?>
        </div>
    </nav>
    ...
    <div class="container-fluid px-4 pt-3">
        ...
        <div class="reset-alert warning"><i class="fa-solid fa-rotate-left me-2"></i>🔄 Tüm sezonlar sıfırlandı. Premier League geçmişi fabrika ayarlarına döndürüldü.</div>
        <?php endif; ?>
    </div>
    <?php endif; ?>
    ...
    <div class="container-fluid py-3 px-4">
        <div style="background:linear-gradient(135deg,#1a0b2e,#3d195b);border:2px solid #e2f89c;border-radius:12px;padding:20px 24px;margin-bottom:16px;text-align:center;">
            <div style="font-family:monospace;color:#e2f89c;font-size:0.82rem;">=============================================</div>
            <div style="font-family:'Oswald',sans-serif;font-size:1.3rem;font-weight:900;color:#fff;margin:8px 0;">🏆 PREMİER LİG <?=$sezon_yili?>/<?=$sezon_yili+1?> SEZONU ŞAMPİYONU 🏆</div>
            <div style="font-family:'Oswald',sans-serif;font-size:1.6rem;font-weight:900;color:#e2f89c;margin:6px 0;">★ <?=htmlspecialchars($sampiyon_takim['takim_adi'])?> ★</div>
            <div style="font-family:monospace;color:#e2f89c;font-size:0.82rem;">=============================================</div>
            ...
            <div style="margin-top:12px;font-size:0.8rem;max-width:400px;margin-left:auto;margin-right:auto;text-align:left;">
                <div style="color:#e2f89c;font-weight:700;margin-bottom:6px;text-transform:uppercase;font-size:0.72rem;">Final Tablosu — İlk 5</div>
                ...
                <div style="display:flex;gap:8px;padding:3px 0;color:#fff;border-bottom:1px solid rgba(255,255,255,0.1);">
                    <span style="width:20px;color:#e2f89c;font-weight:700;"><?=$idx+1?></span>
                    ...
                    <span style="color:#e2f89c;font-weight:900;"><?=$st['puan']?>P</span>
                </div>
                <?php endforeach; ?>
            </div>
            <div style="background:rgba(0,0,0,0.4);border:1px solid rgba(226,248,156,0.3);border-radius:6px;padding:10px 14px;margin-top:12px;font-family:monospace;font-size:0.78rem;color:#a3e635;text-align:left;max-width:400px;margin-left:auto;margin-right:auto;">
                <div style="color:#94a3b8;margin-bottom:4px;">// index.php için son şampiyon güncellemesi</div>
                $son_sampiyon['premier_lig'] = "<?=htmlspecialchars($sampiyon_takim['takim_adi'])?>";
            </div>
        </div>
    </div>
    <?php endif; ?>
